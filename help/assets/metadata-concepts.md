...
## メタデータスキーマに関する参照情報 {#metadata-schemata-reference}
...
例えば制作のプロフェッショナルは、アドビアプリケーションのビルトインの XMP サポートを使用して、複数のファイル形式に情報を渡します。 [!DNL Assets] リポジトリでは、XMP メタデータを抽出し、そのデータをコンテンツのライフサイクルの管理に使用します。自動化ワークフローを作成することもできます。
...
特定のエンティティをリソースと呼びます。つまり、プロパティはリソースに関するものです。 XMP の場合、リソースとなるのは常にアセットです。
...
XMP スキーマは、次を含む共通XML名前空間のプロパティ名のセットです
データタイプと記述情報。 XMP スキーマは、そのXML名前空間URIによって識別されます。 名前空間を使用すると、同じ名前で異なる意味を持つ異なるスキーマ内のプロパティ間の競合を防ぐことができます。
