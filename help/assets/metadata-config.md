...
1. XML名前空間規則でカスタム名前空間を指定します。 IDは、URIとIDに関連付けられたプレフィックスの形式で指定します。 「**[!UICONTROL 保存]**」をクリックします。
...
1. ダイアログで、**[!UICONTROL 削除]**&#x200B;をクリックして、削除操作を確認します。 メタデータプロファイルがリストから削除されます。
...
**[!UICONTROL プロパティにマッピング]**：このプロパティは、フォルダーノードが保存されている CRX リポジトリ内でのフォルダーノードの相対パスを指定します。 パスがフォルダーのノードの下にあることを示す「**./**」で始まります。
