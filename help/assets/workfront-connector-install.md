...
>[!NOTE]
>
> 企業のプロキシサーバーを使用する場合は、[!UICONTROL 拡張 Workfront コネクタ]で認識できるように、[!UICONTROL Apache HTTP コンポーネントプロキシ設定 PID] に [!DNL workfront] を含めます。 必須の PID 形式は `org.apache.http.proxyconfigurator~workfront` です。
...
[!DNL Workfront for Experience Manager enhanced connector] を最新バージョンに更新する手順は次のとおりです。
