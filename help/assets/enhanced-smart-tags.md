...
バックグラウンドでは、Smart Content ServiceがAdobe AIフレームワークを使用して、タグ構造やビジネス分類法に関する画像認識アルゴリズムをトレーニングしています。 その後、このコンテンツインテリジェンスを使用して、アセットの個々のセットに関連性の高いタグが適用されます。
...
フォルダーに対してこのオプションを選択すると、スマートコンテンツサービスによってフォルダー内のアセットが自動的にタグ付けされます。 デフォルトでは、タグ付けワークフローは毎日午前12:00時に実行されます。
