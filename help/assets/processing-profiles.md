...
これらのルールには、メタデータの追加、画像のスマート切り抜き、ビデオエンコーディングプロファイルの構築などが含まれます。 Adobe Experience Manager では、3 種類のプロファイルを作成することができ、その詳細は次のリンクで説明されています。
...
メタデータプロファイル、イメージプロファイル、ビデオプロファイルを作成、編集および削除するには、管理者権限が必要です。

メタデータプロファイル、イメージプロファイル、ビデオプロファイルを作成したら、新しくアップロードするアセットのアップロード先として使用する 1 つ以上のフォルダーにそのプロファイルを割り当てます。

Experience Manager Assetsでのプロファイルの使用に関する重要な概念は、プロファイルがフォルダーに割り当てられていることです。プロファイル内には、ビデオプロファイルまたは画像プロファイルと共に、メタデータプロファイルの形式の設定があります。これらの設定は、フォルダーの内容とそのサブフォルダーの内容を処理します。したがって、ファイルとフォルダーの名前、サブフォルダーの配置方法、およびこれらのフォルダー内のファイルの処理方法は、これらのアセットがプロファイルでどのように処理されるかに大きな影響を与えます。
一貫性のある適切なファイル名やフォルダー名、適切なメタデータを使用することで、デジタルアセットコレクションを最大限に活用し、適切なファイルが適切なプロファイルによって処理されるようにします。
...
>[!NOTE]
>
>Experience Manager 6.4.6.0以降でのみ&#x200B;*Dynamic Media - Scene7 モード*&#x200B;に適用されます。

後で変更した既存の処理プロファイルがあるフォルダー内のアセットを再処理できます。

例えば、画像プロファイルを作成してフォルダーに割り当てたとします。 フォルダーにアップロードした画像アセットには、画像プロファイルが自動的にアセットに適用されます。 ただし、後でプロファイルに新しいスマート切り抜き率を追加することにします。 その場合は、もう一度アセットを選択してフォルダーに再アップロードするのではなく、*Dynamic Media 再処理* <!-- *Scene7: Reprocess Assets* --> ワークフローを実行するだけです。
...
1. Experience Manager のアセットページで、処理プロファイルが割り当てられている、**[!UICONTROL Dynamic Media 再処理]**&#x200B;ワークフローの適用対象となるアセットフォルダーに移動します。
...
1. ワークフローモデルページのカード表示またはリスト表示で、「**[!UICONTROL Dynamic Media 再処理]**」を選択します。
   ...
   ![カード表示で Dynamic Media 再処理ワークフローが選択されたワークフローモデルページ](/help/assets/assets-dm/reprocess-assets7.png)

1. ツールバーの「**[!UICONTROL 編集]**」をクリックします。 新しいブラウザータブに、Dynamic Media 再処理ワークフローモデルページが開きます。
