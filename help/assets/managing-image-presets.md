...
### スマート切り抜き、Adobe Illustrator（AI）、Postscript（EPS）、PDF ファイル形式 {#adobe-illustrator-ai-postscript-eps-and-pdf-file-formats}
...
サムネールのサイズは **`width:height:center`** の形式で定義されます（例：`80:80:false`）。 幅と高さは、サムネールのピクセル単位のサイズを決定します。 中央値はfalseまたはtrueで、trueに設定すると、サムネール画像のサイズが設定で指定されたサイズに正確に反映されます。 画像が指定よりも小さいサイズに変更された場合は、サムネール内で中央揃えされます。
...
* **[!UICONTROL Format]** （**[!UICONTROL Basic]** タブ） - **[!UICONTROL JPEG]**&#x200B;または要件を満たす別の書式を選択します。 すべてのweb ブラウザーはJPEGの画像フォーマットをサポートしています。小さなファイルサイズと画質のバランスが良くなります。 ただし、JPEGの画像では、圧縮設定が低すぎる場合に画像のアーティファクトが不要になる可能性がある非可逆圧縮方式が使用されます。 このため、アドビは圧縮品質を 75 に設定することをお勧めします。 この設定は、画質と小さなファイルサイズのバランスが取れた設定です。
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>メニューから形式を選択します。</p> <p>「<strong>JPEG</strong>」を選択すると、次の追加オプションを入力できます。</p>
    <ul>
     <li><strong>品質</strong> - JPEGの圧縮レベルを制御します。 この設定は、ファイルサイズと画質の両方に影響します。 JPEGの画質は1-100です。 スライダーをドラッグすると、拡大・縮小が表示されます。</li>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td><p>「<strong>なし</strong>」、「<strong>シャープ</strong>」または「<strong>アンシャープマスク</strong>」を選択します。 </p>
    ...
    <ul>
     <li><strong>量</strong> - 端のピクセルに適用されるコントラストを制御します。 デフォルトの実数の値は1.0です。 高解像度の画像の場合は、最大5.0まで拡大できます。 この量は、フィルターの強さを表す尺度と考えてください。</li>
     ...
     <li><strong>しきい値</strong> - アンシャープマスクフィルターが適用されるときに無視するコントラストの範囲を決定します。 つまり、このオプションは、エッジのピクセルと見なされ、シャープ化される前に、シャープ化されたピクセルが周囲の領域とどのように異なっているかを決定します。 ノイズが入らないように、2～20 の範囲で様々な整数値を試してください。 </li>
