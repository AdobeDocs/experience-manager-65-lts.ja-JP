...
このアプローチはAEMで機能しますが、ページとその構成要素が複雑になると問題が発生する可能性があります。 そのような場合、同じJS ライブラリの複数のコピーが最終的なHTML出力に含まれる可能性があります。 これを回避してクライアントサイドライブラリを論理的に整理するために、AEM では&#x200B;**クライアントサイドライブラリフォルダー**&#x200B;を使用します。
...
JS、CSS、またはテーマライブラリのフィルタリングの属性など、完全な情報については、[ui:includeClientLib](/help/sites-developing/taglib.md#lt-ui-includeclientlib)を参照してください。

>[!CAUTION]
>
>以前はクライアント ライブラリを含めるために一般的に使用されていた`<cq:includeClientLib>`は、AEM 5.6以降で非推奨になりました。 [`<ui:includeClientLib>`](/help/sites-developing/taglib.md#lt-ui-includeclientlib)は、上記のように代わりに使用する必要があります。
...
* 結合対象の JS／CSS ソースファイル（いずれかまたは両方）。
* 画像ファイルなど、CSS スタイルをサポートするリソース。

  **メモ：**&#x200B;サブフォルダーを使用してソースファイルを整理できます。
* 生成される JS／CSS ファイルに結合するソースファイルを識別する、1 つの `js.txt` ファイルと 1 つの `css.txt` ファイル（いずれかまたは両方）。
...
1. ソースファイルを任意の方法でライブラリフォルダーに追加します。 例えば、WebDav クライアントを使用してファイルをコピーするか、ファイルを作成してコンテンツを手動でオーサリングします。
...
クライアントライブラリのコードを別のクライアントライブラリに埋め込むことができます。 実行時には、埋め込みライブラリの生成されたJS ファイルおよびCSS ファイルに、埋め込みライブラリのコードが含まれます。
...
AEMには、次のクライアントライブラリカテゴリが含まれています。 特定のサイトの機能に必要なものだけを埋め込む必要があります。 ただし、**このリストの順序は保持する必要があります**。
...
情報には、ライブラリのパスとタイプ（CSSまたはJS）、およびライブラリ属性（カテゴリや依存関係など）の値が含まれます。 ページ上の後続のテーブルには、各カテゴリとチャネルのライブラリが表示されます。
