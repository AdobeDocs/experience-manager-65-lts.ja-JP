...
* [GitHubでaem-sites-example-custom-template-type プロジェクトを開きます](https://github.com/Adobe-Marketing-Cloud/aem-sites-example-custom-template-type)
...
* ページの作成時に初期コンテンツ（`/initial`）と結合されます。
...
* テンプレートの `structure` と `initial` を結合したサブツリーを使用して作成されます。
...
* **コンポーネント**:
  ...
  * ページコンポーネントは、テンプレートの `structure/jcr:content` ツリーとページの `jcr:content` ツリーを結合します。
