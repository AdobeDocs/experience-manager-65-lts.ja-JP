...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>コンポーネント ブラウザー（タッチ操作対応UI）またはSidekick （クラシック UI）でコンポーネントを選択できるグループです。<br /> 値<code>.hidden</code>は、実際の段落システムなど、UIから選択できないコンポーネントに使用されます。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td><code>String</code></td>
   <td>コンポーネントブラウザーまたはサイドキックからコンポーネントを追加するときにコンテンツテンプレートとして使用されるノードのパス。 これは、コンポーネントノードに対する相対パスではなく、絶対パスである必要があります。<br /> 他の場所で既に利用可能なコンテンツを再利用しない限り、これは必須ではなく、<code>cq:template</code>で十分です（以下を参照）。</td>
  </tr>
  <tr>
...
<table>
 <tbody>
  ...
  <tr>
   <td>-</td>
   <td>スペーサーを追加します。<br /> クラシック UIでのみ表示されます。 タッチ操作対応 UI は、アクションがコンテキストメニューに表示されないので、適用されません。</td>
  </tr>
  <tr>
...
### cq:layout （クラシック UIのみ） {#cq-layout-classic-ui-only}
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>デフォルト値。 コンポーネントのエディションには、クリックやコンテキストメニューから「マウスオーバー時」にアクセスできます。<br /> 高度な使用の場合、対応するクライアント側オブジェクトは<code>CQ.wcm.EditRollover</code>です。</td>
  </tr>
  <tr>
   <td><code>editbar</code></td>
   <td>コンポーネントのエディションには、ツールバーからアクセスできます。<br /> 高度な使用の場合、対応するクライアント側オブジェクトは<code>CQ.wcm.EditBar</code>です。</td>
  </tr>
  <tr>
...
### cq:dialogMode （クラシック UIのみ） {#cq-dialogmode-classic-ui-only}
...
### cq:EditConfig子ノードを使用した設定 {#configuring-with-cq-editconfig-child-nodes}
...
例えば、クラス名は、ダウンロードコンポーネントのJSPで次のように定義されます
（`/libs/foundation/components/download/download.jsp`）。`file`は、ダウンロードコンポーネントの編集設定におけるドロップターゲットのノード名です。
...
### cq:actionConfigs （クラシック UIのみ） {#cq-actionconfigs-classic-ui-only}
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>ハンドラーは、コンポーネントが挿入される前にトリガーされます。<br /> タッチ操作対応UIでのみ機能します。</td>
   <td> </td>
  </tr>
  <tr>
