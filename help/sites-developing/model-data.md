...
開発で nt:unstructured （&amp;friends）を愛することを学びます。
...
私の経験では、多くの汎用アプリケーションは、nt:folder および nt:files とのインタラクションを暗黙的に許可し、追加のメタ情報によってエンリッチメントされた場合に、それらのイベントを処理して表示する方法を知っています。 例えば、JCR をベースとしている CIFS や WebDAV のようなファイルサーバー実装との直接のインタラクションは暗黙となります。

私は経験則として、ファイル名と MIME タイプを保存する必要がある場合は、 `nt:file`/ `nt:resource` が良いマッチだと思います。複数の「ファイル」がある場合は :foldernt は保存するのに適した場所です。

リソースのメタ情報を追加する必要がある場合（例えば、「author」プロパティや「description」プロパティなど）、`nt:file` ではなく、`nt:resource` を拡張します。私は nt を伸ばすことはめったになく :file しばしば `nt:resource` を伸ばします。
