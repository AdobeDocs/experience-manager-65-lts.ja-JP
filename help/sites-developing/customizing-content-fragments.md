...
* すべてのコンテンツはアセットの `jcr:content/data` ノードに格納されます。
   ...
   * バリエーションは、バリエーションの名前を含むサブノードの下に保存されます。
例：`jcr:content/data/myvariation`

   * 各エレメントのデータは、エレメント名を持つプロパティとして、各サブノードに格納されます。
例えば、要素`text`のコンテンツは`jcr:content/data/master`にプロパティ `text`として保存されます

* メタデータと関連コンテンツは以下に保存されます `jcr:content/metadata`
ただし、タイトルと説明は、従来のメタデータとは見なされず、`jcr:content`に保存されます
...
* **リソースタイプ**
`sling:resourceTypes`のリストを指定して、コンテンツフラグメントのレンダリングに使用されるコンポーネントと、バックグラウンド処理を適用する場所を定義できます。

* **参照プロパティ**
プロパティのリストを設定して、各コンポーネントのフラグメントへの参照が保存される場所を指定できます。
