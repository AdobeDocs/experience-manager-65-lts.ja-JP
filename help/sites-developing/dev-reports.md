...
>  `N:<name> [<nodeType>]` ：名前が`<*name*>`、ノードタイプが&#x200B;`<*nodeType*>`*のノードを記述します。*
...
**ここで重要なポイントは、クエリの結果セットで返される各ノードが、レポート上の1つの行（つまり1:1の関係）を生成するために使用されることです。**
...
* 抽出した値の解決。様々な方法で行うことができます。
  ...
  例えば、パスをタイトルにマッピングできます（各&#x200B;*jcr:title* プロパティの人間が読みやすいコンテンツのように）。
...
* `settings`
  ...
  * `active`
    ...
    複数の設定を定義できるので、これを使用してどれが現在アクティブなものかを定義できます。 これらはノードの配列で定義されます（これらのノードに対する強制的な命名規則はありませんが、標準レポートでは`0`、`1`..`x`を使用することが多いです）。各ノードには次のプロパティがあります。
...
* `definitions`
  ...
  定義は、ノードの配列（多くの場合、`0`、`1`..`x`）を使用して指定され、それぞれ次のプロパティを持ちます。
...
* `property`
  ...
  * 使用可能なjcr:lastModified プロパティがあるかどうかを確認し、そのプロパティを使用します。
  * 使用できるjcr:lastModified プロパティがない場合は、代わりにjcr:createdの内容が使用されます。
...
* `resolverConfig`
  ...
  * `page`
    ...
    * `propertyName`（オプション）
      ...
      値の解決に使用するプロパティの名前を定義します。 指定しない場合は、デフォルト値の&#x200B;*jcr:title* （ページタイトル）が使用されます。`page` リゾルバーの場合、最初にパスがページパスに解決され、次にページタイトルに解決されます。
...
対応するページのjcr:content （子）ノード上のjcr:description プロパティへのパス値を解決します。
...
* `format`
  ...
  * `duration`
    ...
    指定された 2 つの日時の間のタイムスパンです。 例えば、2/13/11 11 11:23hから1時間かかったワークフローアクションの開始と終了、および2/13/11 1 12:23hで1時間後に終了したワークフローアクションの開始と終了です。
...
1. 最初の列（columnbase）コンポーネントを定義します。 例えば、`/apps/cq/reporting/components/osgireport` の `bundlecol[cq:Component]` です。
   ...
   * 指定されたイベントで更新
   ...
   >[!NOTE]
   >
   >この例では、`N:data` と `P:clientFilter` の定義はありません。 これは、サーバーから受信した値が1:1で返されるためです。これは、デフォルトの動作です。
   >
   >これは、定義と同じです。
   >
   >```
