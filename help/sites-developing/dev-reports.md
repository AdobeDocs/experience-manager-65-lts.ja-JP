...
**ここで重要なのは、クエリの結果セットで返される各単一ノードが、レポートに単一の行を生成するために使用されるということです（つまり、1:1 関係）。**
...
* `property`
   ...
   * 使用できる jcr:lastModified プロパティがない場合は、代わりに jcr:created のコンテンツが使用されます。
...
対応するページの jcr:description （子）ノードの jcr:content プロパティに、パス値を解決します。
...
* `format`
   ...
   * `duration`
     ...
     指定された 2 つの日時の間のタイムスパンです。例えば、ワークフローアクションの開始から終了までの時間を 1 時間とした場合、開始を 2011 年 2 月 13 日とし :23h 終了を 2011 年 2 月 13 日 12 日と :23h ます。
...
1. 最初の列（columnbase）コンポーネントを定義します。例えば、`/apps/cq/reporting/components/osgireport` の `bundlecol[cq:Component]` です。
   ...
   * 指定されたイベントで更新
   ...
   >[!NOTE]
   >
   >この例では、`N:data` と `P:clientFilter` の定義はありません。これは、サーバーから受け取った値が 1:1 ベースで返されるためで、これがデフォルトの動作です。
   >
   >これは、定義と同じです。
   >
   >```
