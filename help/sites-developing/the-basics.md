...
Slingを使用すると、レンダリングするコンテンツのタイプは最初の処理の検討ではありません。 代わりに、主な考慮事項は、URLがレンダリングを実行するためにスクリプトを見つけることができるコンテンツオブジェクトに解決されるかどうかです。 これにより、web コンテンツ作成者は、要件に合わせて容易にカスタマイズできるページを作成できます。
...
Sling では、処理はユーザーリクエストの URL によって実行されます。 これにより、適切なスクリプトによって表示されるコンテンツが定義されます。 これを行うには、情報はURLから抽出されます。
...
リクエストが分解され、必要な情報が抽出されます。 リポジトリは、リクエストされたリソース（コンテンツノード）で検索されます。
...
* GET/HEAD リクエストおよび .html で終わる URL（デフォルトのリクエストタイプ、デフォルトの形式）
  ...
  スクリプトは/apps/hr/jobs/jobs.espです。Sling:resourceTypeの最後のセクションでファイル名を指定します。
...
これを行うには、sling:include （&quot;/&lt;path>/&lt;resource>&quot;） コマンドを使用します。 これにより、参照されるリソースの定義を効率的に含めることができます。例えば、次のステートメントでは、画像をレンダリングするために既存の定義を参照しています。
...
コンテンツノードがレンダリングを制御します。 Sling は、受信リクエストからコンテンツノードを取得します。 このノードのプロパティ sling:resourceTypeは、使用するSling レンダリングコンポーネントを指しています。
...
ページには、タイプ cq:Pageの階層ノードとタイプ cq:PageContentのコンテンツノードがあります。 コンテンツノードのプロパティ sling:resourceTypeは、ページのレンダリングに使用されるページコンポーネントを指します。
...
AEM では、本番環境は多くの場合、[オーサーインスタンスとパブリッシュインスタンス](/help/sites-deploying/deploy.md#author-and-publish-installs)という 2 種類のインスタンスで構成されます。
