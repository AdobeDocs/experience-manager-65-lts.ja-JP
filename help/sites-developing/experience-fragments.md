...
`/libs/cq/experience-fragments/components/xfpage/xfpage.html`がないので、次の状態に戻ります。
...
>[!NOTE]
>
>リンクは常に公開インスタンスを参照します。 サードパーティはオーサリングインスタンスではなく、パブリッシングインスタンスからリンクを常に呼び出します。
>
>詳しくは、[URLの外部化](/help/sites-developing/externalizer.md)を参照してください。

![xf-14](assets/xf-14.png)

プレーンレンディションセレクターは、追加のスクリプトではなくトランスフォーマを使用します。[`Sling Rewriter`](https://sling.apache.org/documentation/bundles/output-rewriting-pipelines-org-apache-sling-rewriter.html)はトランスフォーマとして使用され、次のように設定されます。
...
1. `html`、`head`、`body` の各要素が `div` 要素に置き換わります。 `meta`、`noscript`および`title`要素は削除されます（これらは元の`head`要素の子要素であり、`div`要素に置き換えられたときに考慮されません）。
...
1. AEM では、HTML に存在するすべての内部リンクを変更して、公開されたリソースを指すようにします。
   ...
   これらの属性のリンクは、公開されたインスタンス上にあるかのように、公開されたインスタンス上にあるかのようにURLを再作成するために、AEM Link Externalizer `publishLink()`を通過します。
...
* リソースに対するメカニズムが`sling:alias`個あります。
...
* `link`：
処理中のリンクの `String` 表現です。 通常、オーサーインスタンス内のリソースを指す相対URL。
...
例えば、URLの`/etc.clientlibs`部分を削除し、適切な外部ドメインを追加します。 話を簡単にするために、`rewriteLinkExample2` に示すように、サービスのリソースリゾルバーにアクセスできるとします。
