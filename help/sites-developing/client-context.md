...
* コンポーネント JSP スクリプトにパーソナライゼーション :storeRendererTag タグを追加して、セッションストアの名前を識別します。
...
1. オーサーインスタンス （[https://localhost:4502/content/geometrixx-outdoors/en.html](https://localhost:4502/content/geometrixx-outdoors/en.html)）でGeometrixx Outdoors ホームページを開きます。
...
`path`
ページテンプレートに含まれる、クライアントコンテキストコンポーネントのデザインスタイルで使用されます。例えば、次の場所を標準として使用します。
