...
- **フルインストーラーのアップグレードサポート**：フルインストーラーベースのアップグレードは、AEM 6.5.23.0からのみサポートされます。
...
| Z：サポート対象外 | この設定はサポートされていません。 Adobeでは、設定が機能するかどうかに関する記述は行われず、設定がサポートもされていません。 |
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>Microsoft® SQL Server JDBC ドライバー12.10.0<br /> </p> <p>sqljdbc12.10.0.jar</p> </td>
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>Red Hat® Enterprise Linux® 9 （Kernel 5.x）（64 ビット）</p> </td>
   <td><p>A：サポート対象</p> </td>
  ...
  <tr>
   <td><p>SUSE® Linux® Enterprise Server 15-SP6 （64 ビット）</p> </td>
   <td><p>A：サポート対象</p> </td>
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>Microsoft® Windows® 11 64 ビット</p> </td>
   <td>E：動作する見込み</td>
   <td><p>サービスパックと重要なアップデート</p> </td>
  </tr>
 </tbody>
...
- JEE版AEM Formsでは、指定したメジャーバージョンとマイナーバージョンのサポート対象ソフトウェアに加えて、アップデートとパックの修正が行われます。 ただし、次のメジャーバージョンまたはマイナーバージョンに対するアップデートは、特に記載がない限りサポートされていません。
...
- AEM Forms on JEE では、弊社の[サードパーティソフトウェアサポートポリシー](#third-party-patch-support-policy-third-party-patch-support-policy)に従い、様々なサードパーティソフトウェアをサポートしています。
...
AEM Forms アプリでApache Cordovaがサポートされるようになりました。 サポートされているCordovaのプラットフォーム固有のバージョンは次のとおりです。
...
>[!NOTE]
>
>その他の要件については、「[JEE上のシングルサーバーAEM Formsの必要システム構成](https://helpx.adobe.com/content/dam/help/en/experience-manager/65LTS/forms/prepare-install-single-server.pdf)」を参照してください
...
>[!NOTE]
>
>Acrobat DC 製品ファミリーでは、異なる製品である Acrobat と Reader のそれぞれに、「Classic」と「Continuous」の 2 種類のトラックが用意されています。
...
<table>
 <tbody>
  ...
  <tr>
   <td><p>Microsoft® Windows® 11 （Enterprise、Pro、Basic）</p> <p>64 ビット版</p> <p> </p> </td>
   <td>サービスパックと重要なアップデート</td>
  </tr>
  <tr>
...
>- Acrobat DC 以降のバージョンでは、Workspace は Macintosh OS X 10.6 および 10.7 上の Safari 5.1 をサポートしています。
...
場合によっては、主要な機能を変更しそのために完全な後方互換性をサポートしなくなったサードパーティアップデートはサポートされません。
