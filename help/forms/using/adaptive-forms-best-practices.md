...
* アダプティブフォーム、テーマ、テンプレート、クラウド設定などのアセットのオーサリングは、オーサーノードでのみ実行され、設定されたパブリッシュノードで公開できます。
詳しくは、[&#x200B; フォームとドキュメントの公開と非公開](/help/forms/using/publishing-unpublishing-forms.md)を参照してください
* Forms アドオンパッケージは、ドキュメントサービスの操作をサポートするためにオーサリングおよびパブリッシングに必要であるため、依存関係と見なすことができます。
Forms関連のサンプルテンプレート、テーマ、DOR パッケージのみを使用する場合は、[aemforms-references-* パッケージ &#x200B;](/help/forms/using/upgrade-forms-osgi.md)からダウンロードできます。
