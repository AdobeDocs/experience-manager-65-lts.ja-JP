...
AEM as a Cloud Serviceのドキュメントについては、[Cloud Service上のAEM Forms](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/forms-overview/aem-forms-cloud-service-architecture.html?lang=ja)を参照してください。
...
以下の図には、AEM Forms の一般的なデプロイメント環境で使用される AEM Forms サーバーの様々な設定とそのコンポーネントが表示されています。

![aem_forms_-_recommendedtopology](assets/aem_forms_-_recommendedtopology.png)

**作成者：** オーサーインスタンスは、標準オーサー実行モードで実行されているAEM Forms サーバーです。<!--It can be AEM Forms on JEE or AEM Forms on OSGi environment.--> これは、内部ユーザー、フォームとインタラクティブなコミュニケーションのデザイナー、開発者を対象としています。 次の機能が有効になります。

* **フォームとインタラクティブなコミュニケーションの作成機能と管理機能：**&#x200B;設計者と開発者は、アダプティブフォームやインタラクティブなコミュニケーションの作成および編集したり、外部で作成された他のタイプのフォームをアップロードしたりできます。例えば、Adobe Forms Designer で作成されたフォームなど、Forms Manager コンソールを使用してこれらのアセットを管理します。
* **フォームとインタラクティブなコミュニケーションの公開機能：**&#x200B;オーサーインスタンス上でホストされるアセットをパブリッシュインスタンスに公開して、ランタイム操作を実行することができます。 AEM のレプリケーション機能を使用して、アセットが公開されます。 パブリッシュしたフォームを処理インスタンスに手動でプッシュするために、レプリケーションエージェントをすべてのオーサーインスタンス上で設定すること、および受信したフォームをパブリッシュインスタンスに自動で複製するために、別のレプリケーションエージェントを&#x200B;*受信時*&#x200B;トリガーが有効になっている処理インスタンス上で設定することをお勧めします。
...
### データ取得機能、インタラクティブなコミュニケーション機能、OSGi 上のフォーム中心ワークフロー機能を使用する場合のトポロジー {#topology-for-data-capture-interactive-communication-form-centric-workflow-on-osgi-capabilities}

アダプティブフォーム、HTML5 フォーム、PDF フォームなど、AEM Forms のデータ取得機能を使用する場合は、以下のようなトポロジを構成することをお勧めします。 インタラクティブなコミュニケーション機能と OSGi 上のフォームベースワークフロー機能を使用する場合も、このトポロジーを構成することをお勧めします。例えば、ビジネスプロセスワークフローで AEM インボックスと AEM Forms アプリケーションを使用する場合などです。
