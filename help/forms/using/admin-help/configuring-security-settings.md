---
title: セキュリティ設定の指定
description: セキュリティ設定の指定方法について説明します。 アクセスを制限することで PDF ドキュメントを保護できます。 ドキュメントを暗号化、認証またはパスワードで保護できます。
contentOwner: admin
...
*セキュリティ設定*&#x200B;では、そのセキュリティ設定を使用して変換されるファイルのセキュリティとアクセス許可を制御します。
...
これらの設定では、互換性と暗号化を指定します。 セキュリティ設定へのアクセス方法については、[セキュリティ設定の作成または編集](configuring-security-settings.md#create-or-edit-a-security-setting)を参照してください。
...
以前のバージョンの Acrobat では、より高い互換性設定が設定されている PDF ドキュメントを開くことができません。 例えば、「Acrobat 7.0 以降」オプションを選択した場合、Acrobat 6.0 以前ではドキュメントを開くことはできません。

互換性レベルが、同じソースの PDF 互換性レベルと一致していることを確認してください。 例えば、Acrobat 5.0 以降と互換性のある標準 PDF 設定を使用するようにウォッチフォルダーが設定されている場合、セキュリティ互換性レベルは Acrobat 5.0 より高くてはなりません。
...
**メタデータを除くすべてのドキュメントコンテンツを暗号化（Acrobat
6以降と互換性があります）:**&#x200B;文書の内容を暗号化しますが、検索エンジンが文書メタデータにアクセスできるようにします。 このオプションは、「互換性」オプションが Acrobat 6.0 以降、Acrobat 7.0 以降または Acrobat 9.0 以降に設定されている場合にのみ使用できます。

**添付ファイルのみを暗号化（Acrobat 7以降）
互換性）:** ユーザーはパスワードなしでドキュメントを開くことができますが、添付ファイルを開くにはパスワードを入力する必要があります。 このオプションは、「互換性」オプションが Acrobat 7.0 以降または Acrobat 9.0 以降に設定されている場合にのみ使用できます。

これらの設定により、パスワードのセキュリティが構成されます。

>[!NOTE]
>
>パスワードを忘れた場合、ドキュメントからパスワードを回復することはできません。 パスワードを忘れた場合に備えて、別の安全な場所にパスワードを保存することをお勧めします。 また、パスワードで保護されていないドキュメントのバックアップコピーを保管してください。

**ドキュメントを開く際にパスワードが必要：** パスワードオプションを有効にします。

**ドキュメントを開くパスワード：**&#x200B;ユーザーは、指定したパスワードを入力しない限りドキュメントを開くことができません。 パスワードでは大文字と小文字が区別されます。 Acrobat は、RSA Security Inc. の RC4 セキュリティ方式を使用して PDF ドキュメントをパスワード保護します。 印刷や編集を制限している場合は、セキュリティを強化するためにドキュメントを開くパスワードの追加をお勧めします。
...
**パスワードを使用して印刷と編集を制限する
ドキュメントとそのセキュリティ設定：**&#x200B;は、権限に対する制限を有効にします。

**権限パスワード：** ユーザーが印刷および編集できないように制限します。 ユーザーは、指定したパスワードを入力しない限り、これらのセキュリティ設定を変更できません。 ドキュメントを開くパスワードで使用している同じパスワードは使用できません。 権限パスワードを設定すると、そのパスワードを入力したユーザーだけがセキュリティ設定を変更できます。 PDF ドキュメントが両方のタイプのパスワードを持っている場合、いずれかのパスワードでドキュメントが開きます。 ただし、制限されている機能は、権限パスワードでのみ設定または変更できます。 PDF ドキュメントが権限パスワードのみを持つ場合またはユーザーがドキュメントを開くパスワードを使用してドキュメントを開く場合は、ユーザーがセキュリティ設定を変更しようとすると、パスワードの入力を求めるプロンプトが表示されます。
...
**フォームフィールドに入力し、既存の署名に署名する
フィールド：** ユーザーがフォームに入力し、デジタル署名を追加できます。 ただし、ユーザーは、コメントを追加またはフォームフィールドを作成することはできません。 このオプションは、高い暗号化レベル（Acrobat 5.0、6.0、7.0 または 9.0）が選択されている場合にのみ使用できます。

**コメント、フォームフィールドへの入力、既存の署名
署名フィールド：** ユーザーがフォームに入力し、デジタル署名とコメントを追加できます。

**ページレイアウト、タッチアップ、フォームフィールドへの入力、署名
既存の署名フィールド：** ユーザーがページを挿入、回転、または削除し、ブックマークまたはサムネール画像を作成し、フォームに入力し、デジタル署名を追加できます。 このオプションでは、ユーザーはフォームフィールドを作成できません。 このオプションは、低い暗号化レベル（Acrobat 3.0）が選択されている場合にのみ使用できます。
