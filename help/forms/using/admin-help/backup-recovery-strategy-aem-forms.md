...
Adobe Experience Manager（AEM）は AEM Forms の統合機能の一部です。 したがって、AEMをAEM Formsのバックアップと同期させてバックアップする必要があります。Correspondence Management ソリューションやForms Managerなどのサービスは、AEM FormsのAEMに保存されているデータに基づいています。データの損失を防ぐには、AEM forms固有のデータをバックアップして、GDSとAEM（リポジトリ）がデータベース参照と確実に関連付けられるようにする必要があります。データベース、GDS、AEM、Content Storageのルート ディレクトリは、元のディレクトリと同じDNS名を持つコンピューターに復元する必要があります。
...
データベースは、フォームアーティファクト、サービス設定、プロセスの状態、および GDS ファイルへのデータベース参照を保存するために使用されます。 データベースへのドキュメントの保存を有効にすると、GDS 内の永続データと永続ドキュメントもデータベースに保存されます。 データベースは、次の方法でバックアップおよび回復できます。
...
>[!NOTE]
>
>この状況は、このスクリプトを使用して GDS の場所を変更する必要がある唯一の状況です。 AEM Forms の実行中に GDS の場所を変更するには、管理コンソールを使用します。 （[一般的なAEM フォームの設定](/help/forms/using/admin-help/configure-general-aem-forms-settings.md#configure-general-aem-forms-settings)*を参照）。 *
