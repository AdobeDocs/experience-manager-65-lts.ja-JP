...
1. 「サービスプロバイダーのエンティティ ID」ボックスに、AEM Forms サービスプロバイダー実装の識別子として使用する一意の ID を入力します。 この一意のIDは、IDPの設定時にも指定します（例：`um.lc.com`）。 AEM フォームへのアクセスに使用されるURL （例：`https://AEMformsserver`）を使用することもできます。
