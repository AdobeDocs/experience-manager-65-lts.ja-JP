---
title: PDF 生成で多数の PDF をワークベンチで印刷できない
description: お客様がワークベンチを通じて実装されたサービスを使用して多数の PDF を生成すると、印刷サービスは失敗します。
...
# PDF 生成で多数の PDF をワークベンチで印刷できない {#PDF-generation-fails-to-print-a-large-number-of-PDFs-via-WorkBench}

## 問題 {#issue}

お客様がワークベンチを通じて実装されたサービスを使用して多数の PDF を生成する場合。 サービスはメモリ不足のため失敗します。 エラーは、次のように表示されます。
