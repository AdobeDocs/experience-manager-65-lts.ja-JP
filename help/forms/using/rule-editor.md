...
AEM as a Cloud Serviceのドキュメントについては、[Cloud Service上のAEM Forms](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-foundation-components/add-rules-and-use-expressions-in-an-adaptive-form/rule-editor.html?lang=ja)を参照してください。
...
ルールエディターで使用可能なルールタイプついて詳しくは、[ルールエディターで利用できるルールタイプ](#available-rule-types-in-rule-editor)を参照してください。
...
「**移動先**」は、他のインタラクティブなコミュニケーション、他のアダプティブフォーム、画像やドキュメントフラグメントなどの他のアセット、または外部 URL に移動します。 詳しくは、 [インタラクティブなコミュニケーションにボタンを追加する](../../forms/using/create-interactive-communication.md#addbuttontothewebchannel)を参照してください。
...
1. ルールを記述するフォームオブジェクトを選択し、コンポーネントツールバーで ![edit-rules](assets/edit-rules.png) をクリックします。 ルールエディターのユーザーインターフェイスが表示されます。
...
1. **[!UICONTROL アダプティブフォームとインタラクティブなコミュニケーションの Web チャネル設定]**&#x200B;をクリックして編集します。
...
または、`@argument` `{type} name <Parameter Description>` **または** `@arg` `{type}` `name <Parameter Description>`を使用することもできます。
関数で使用されるパラメーターを表示します。 関数には、複数のパラメータタグを設けることができます。各パラメーターは、実行順序に応じて 1 個のタグを設けることができます。
  ...
  他のすべてのパラメータータイプは、上記のいずれかに分類されます。 「なし」はサポートされていません。 上記のタイプのいずれかを選択していることを確認してください。 タイプでは大文字と小文字が区別されません。 パラメーター `name`. `<Parameter Descrption>` `<parameter>  can have multiple words. </parameter>` では、スペースは使用できません。
...
フォームオブジェクトを選択し、続けて￥![edit-rules1](assets/edit-rules1.png)」を選択すると、オブジェクト上に既存のルールが一覧表示されます。 タイトルを表示し、ルール概要をプレビューできます。 さらに、UI を使用すると、ルールの概要の展開および表示、ルールの順序変更、ルールの編集、ルールの削除を行えます。
...
ルール内で条件をドラッグアンドドロップして編集することもできます。 条件の前のハンドル（![handle](assets/handle.png)）を選択し続けます。 次に示すようにポインターがハンドシンボルになったら、ルール内の任意の場所に条件をドラッグ＆ドロップします。 ルール構造が変化します。
...
* ラジオボタン「**既存のGeometrixxのお客様ですか？**」を選択すると、「はい」と「いいえ」のオプションが表示されます。 「はい」の値は **0**、「いいえ」の値は **1** です。
...
注文書フォームでは、次のテーブルが表示されます。ユーザーはこのテーブルに注文を入力します。 このテーブルの内容：
...
上の例で説明した注文書フォームでは、価格が 10000 を超える商品について、1 個を超える数量は発注が制限されています。 これを行うには、以下に示すように検証ルールを記述します。
