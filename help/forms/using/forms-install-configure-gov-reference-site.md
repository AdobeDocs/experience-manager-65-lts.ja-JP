---
title: We.GovおよびWe-Finance リファレンスサイトの設定と設定
...
### インストール前提条件 {#installation-prerequisites}
...
* AEM Forms のインタラクティブなコミュニケーション（印刷と web チャンネル）
...
### ソフトウェアの配布から {#from-software-distribution}
...
新しいパッケージバージョンをインストールするには、4.1と4.2で定義されている手順に従います。 古いバージョンが既にインストールされている場合でも、新しいパッケージバージョンをインストールできます。 ただし、Adobeでは、古いパッケージのバージョンを最初にアンインストールすることをお勧めします。 これを行うには、次の手順を実行します。
...
1. 確認したら、**アンインストール**&#x200B;をもう一度選択し、アンインストールプロセスを完了させます。
...
1. ドロップダウン **We.Gov Forms Users**&#x200B;から選択します。

   ![ワークフローユーザーのグループ設定の編集](assets/edit_group_settings.jpg)

1. メニューバーの&#x200B;**保存して閉じる**&#x200B;をクリックします。
1. **analytics**&#x200B;を検索し、**Analytics Administrators** グループを選択し、**We.Gov Forms Users** グループをメンバーとして追加して、手順2 ～ 7を繰り返します。
1. **forms users**&#x200B;を検索し、**forms-power-users** グループを選択し、**We.Gov Forms Users** グループをメンバーとして追加して、手順2 ～ 7を繰り返します。
1. **forms-users**&#x200B;を検索し、**forms-users** グループを選択し、今回は&#x200B;**We.Gov Users** グループをメンバーとして追加して、手順2 ～ 7を繰り返します。

### メールサーバーの設定 {#email-server-configuration}

1. 設定ドキュメントの確認 [メール通知の設定](/help/sites-administering/notification.md)
1. 管理者としてログインして、このタスクを実行します。
...
1. 任意の SMTP サーバーに接続するようにサービスを設定します。
   ...
   1. **アドレス**&#x200B;から：aemformsdemo@adobe.com
...
1. 「**次へ**」をクリックします。
1. HTTPS ホスト名を&#x200B;*localhost*&#x200B;に設定します。
1. Portを、システムが公開したポートに設定します。
...
1. 次の URL に移動します。
   ...
   *https://&lt;aemserver>:&lt;port>/libs/adobesign/cloudservices/adobesign.html/conf/we-gov*
   ...
   >[!NOTE]
   >
   >設定の問題を回避するには、AEM サーバーへのアクセスに使用するURLが、Adobe Sign OAuth リダイレクト URIで設定されたURLと一致していることを確認します。
   >例：*https://&lt;aemserver>:&lt;port>/mnt/overlay/adobesign/cloudservices/adobesign/properties.html*

1. **We.gov Adobe Sign**&#x200B;設定を選択します。
1. 「**プロパティ**」をクリックします。
1. 「設定」タブに移動します。
1. oAuth URLを入力します。 例えば、`https://secure.na1.echosign.com/public/oauth`はイラスト用です。
1. 設定した Adobe Sign インスタンスから、設定したクライアント ID とクライアント秘密鍵を指定します。
1. 「**Adobe Signに接続**」をクリックします。
1. 接続が成功したら、**保存して閉じる**&#x200B;をクリックして、統合を完了します。
...
1. 以下の詳細を入力します。
   ...
   1. 「C **OAuthに接続」をクリックします**。

1. 認証が成功したら、**保存して閉じる**&#x200B;をクリックして、統合を完了します。
...
1. 設定ビューで「**Keys**」をクリックして、新しいキー（AEMのクライアントシークレットとして使用）を表示します。

   1. キーのコピーは必ず保管してください。後でAzureまたはAEMで表示することはできません。
...
1. 更新およびアクセストークン URLの詳細は、次の場所で確認できます。
...
クラウド設定が完了したら、フォームデータモデルをテストする必要がある場合があります。
...
1. 「**ソースを更新**」タブに移動します。
1. **コンテキストに応じた設定**&#x200B;が`/conf/we-gov`に設定され、設定されたデータソースが`ms-dynamics-odata-cloud-service`であることを確認します。

   ![設定済みのデータソース](assets/configured_data_source.jpg)

1. フォームデータモデルを編集します。

1. サービスをテストして、設定されたデータソースに正常に接続できることを確認します。
   ...
   >[!NOTE]
   >
   >データソースをFDMに正常にバインドするには、AEM Serverの再起動が必要であることが報告されています。

   >[!NOTE]
   >
   >Adobeでは、`Ctrl + C` コマンドを使用してSDKを再起動することをお勧めします。 別の方法を使用してAEM SDKを再起動する。 例えば、Java プロセスを停止すると、AEM開発環境で不整合が発生する可能性があります。

#### Forms データモデル（Derby）のテスト {#test-fdm-derby}

クラウド設定が完了したら、Forms データモデルをテストする必要がある場合があります。
...
1. [サービスのテスト](work-with-form-data-model.md#test-data-model-objects-and-services)を行い、設定したデータソースに正常に接続できることを確認します。
   ...
   * 接続をテストするには、**HOMEMORTGAGEACCOUNT** を選択し、get サービスを実行します。 サービスとシステム管理者が、取得されるデータを確認できるかどうかをテストします。
...
1. 同じページで、フレームワーク設定を更新する場合は、「We.Gov Adobe Analytics Framework （Analytics Framework）」をクリックします（[AEM オーサリングを有効にする](../../forms/using/forms-install-configure-gov-reference-site.md#enableauthoring)を参照）。
...
1. ページの下部までスクロールすると、ページの下部にユーザーの認証情報が表示されます。
   ![アクセスを管理](assets/aftia-admin-user-access.jpg)
1. ユーザー名と共有シークレット情報は、権限ボックスの右側に表示されます。
1. ユーザー名には、名前の中にコロンが含まれています。 コロンの左側にあるすべての情報はユーザー名で、コロンの右側にあるすべての情報は会社名です。次の例を参照してください。
...
>[!NOTE]
>
>`we-gov-forms.ui.analytics-<version>.zip` パッケージをインストールすると、AEM Forms Analytics データはオフラインで使用できるか、Adobe Analytics Cloud設定なしで使用できます。 AEM Sites データには、アクティブなクラウド設定が必要です。

1. *https://&lt;aemserver>:&lt;port>/sites.html/content*&#x200B;に移動します。
1. **AEM Forms We.Gov サイト**&#x200B;を選択して、サイト ページを表示します。
1. サイトページの 1 つ（例：ホーム）を選択し、「**分析とレコメンデーション**」を選択します。

   ![分析とレコメンデーション](assets/analytics_recommendations.jpg)

1. このページには、Adobe Analyticsから取得した情報が表示されます。 この情報は、AEM Sites ページに関連しています。 設計上、この情報はAdobe Analyticsから定期的に更新され、リアルタイムでは表示されません。

   ![AEM Sites 分析](assets/sites_analysis.jpg)

1. ページビューページ（手順3でアクセス）に戻り、表示設定を変更して&#x200B;**リストビュー**&#x200B;の項目を表示することで、ページビュー情報を表示することもできます。
...
>[!NOTE]
>
>AEM Forms Analytics データは、オフライン中またはAdobe Analytics Cloud設定なしで使用できます。 この機能は、`we-gov-forms.ui.analytics-<version>.zip` パッケージがインストールされている場合に当てはまります。 ただし、AEM Sites データにはアクティブなクラウド設定が必要です。
...
Adobe Formsを使用してAEM Formsをインストールおよび設定するには、コンバージョンツールのユーザーに次の情報が必要です。

1. Adobe Developer へのアクセス権限。

1. Adobe Forms コンバージョンサービスとの統合を作成する権限。
...
>[!NOTE]
>
>このセクションで作成された証明書は、Adobe I/Oで統合サービスを作成するために使用されます。ユーザーが統合サービスを作成したら、Adobe I/Oからその情報を使用して設定を完了できます。
...
1. 正しいグループに属していることを確認します（右上のドロップダウンリスト）。
...
1. AEM内のIMS設定に移動します（**Tools** > **Security** > **IMS**）
...
1. システムのサービス URLは`https://aemformsconversion.adobe.io/`に設定されています。
...
1. コンバージョンプロセスが開始されると、ユーザーには次の情報が表示されます。

   ![変換設定](assets/aftia-conversion-in-progress.jpg)

1. コンバージョンが完了すると、ユーザーには次の情報が表示されます。
...
1. すべてのユーザーに対してテンプレート フォルダーの`jcr:read`が有効になっていることを確認してください。 権限が設定されていない場合、サービスユーザーはリポジトリからテンプレートを読み取ることができず、変換が失敗します。
...
これらのテンプレートには、次の場所にあるコンポーネントを使用して作成および構成された、AEM サイト、アダプティブフォーム、インタラクティブなコミュニケーションの各テンプレートが含まれます。
...
このパッケージに含まれる編集可能なテンプレートには、ページネーションやスタイル設定などに Bootstrap 4 の CSS クラスを使用するテンプレートポリシーやページポリシーも事前に設定されています。 すべてのクラスがテンプレートポリシーに追加されたわけではありませんが、Bootstrap 4でサポートされているすべてのクラスをポリシーに追加できます。 使用可能なクラスのリストについては、次を参照してください。
...
これらのフォームは、特定のユースケースに合わせてカスタマイズできます。 フォームが引き続き正しく機能するように、特定のフィールドと送信ロジックを編集しないでください。例えば、次のような操作を行います。

**健康増進のための登録申請：**

* contact_id - 送信時に MS Dynamics の連絡先 ID を受け取るために使用される非表示フィールド
* 送信 – 送信ボタンのロジックには、コールバックをサポートするためのカスタマイズが必要です。 このドキュメントでは、カスタマイズについて説明します。 ただし、フォームを送信し、Forms データモデルを使用して`POST`と`GET`の両方の操作をMS® Dynamicsに実行するには、大きなスクリプトを作成する必要があります。
...
アダプティブフォームは、スタイルエディターまたはテーマエディターを使用してスタイル設定することもできます。
...
登録アダプティブ Formsは、処理のためにOSGI ワークフローに送信されます。 このワークフローは、*https://&lt;aemserver>:&lt;port>/conf/we-gov/settings/models/we-gov-process.html*&#x200B;にあります。
...
* 登録アダプティブフォームは、フォームの電子メールフィールドで指定された電子メールにAdobe Sign電子メールを送信します。 この電子メールアドレスは、Adobe Sign Cloud設定の設定に使用した電子メールと同じ電子メールアドレスにすることはできません。

### 既知の問題 {#known-issues}

* （AGRS-120）サイトナビゲーションコンポーネントは現在、2 レベル以上の深さのネストされた子ページをサポートしていません。
* （AGRS-159）現時点では MS Dynamics FDM は操作を 2 回実行する必要があり、まず、登録アダプティブフォームのデータを Dynamics に POST し、次に連絡先 ID を取得するためにユーザーレコードを取り込みます。 現在の状態では、同じ名前を持つ2人以上のユーザーがDynamicsに存在し、登録アダプティブフォームが送信できない場合、連絡先IDの取得が失敗します。
...
インストールが完了したら、Chrome Browser内でテストするページを読み込みます。 複数のタブを開いていると、スコアに影響を与える可能性があります。 そのため、1つのタブのみを開くことが望ましいです。 ページが読み込まれたら、**ページ上で**&#x200B;を右クリックし、**監査** タブを選択します。 ここでは、開発者は、アクセシビリティプラグインが実行する監査タイプを選択します。 必要なオプションを選択した後、**レポートを生成**&#x200B;をクリックしてPDF ドキュメントを作成します。 PDFには、全体的なアクセシビリティの評価と、アクセシビリティの全体的な評価を上げるために使用できる内容が表示されます。
...
このデータを書き出す場合は、画面の右側にある3つのボタンをクリックし、使用可能なオプションから選択します。

![アクセシビリティレポート](assets/aftia-accessibility-report.jpg)

### Ultramarine テーマ {#ultramarine-theme}

Adobeによって管理されている一般公開されているUltramarine テーマは、に組み込まれています。
インストール可能なZIP ファイル：`we-gov-forms.pkg.all-<version>.zip` このパッケージは、CRXを使用してインストールされます。

Package Manager ユーザーは、**Forms** > **Themes** > **Reference Themes** > **Ultramarine-Accessible**&#x200B;に移動することで、AEM FormsのUltramarine テーマにアクセスできます。
...
We.Gov リファレンスサイトを探索するように設定されています。 We.Gov リファレンスサイトのワークフローと手順について詳しくは、[We.Gov リファレンスサイトのチュートリアル](../../forms/using/forms-gov-reference-site-user-demo.md)を参照してください。
