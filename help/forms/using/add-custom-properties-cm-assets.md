...
1. 新しく作成したオーバーレイノードの下（/apps/fd/cm/ma/gui/content/cmmetadataproperties/commonproperties/col1/items）
型nt:unstructuredのドロップダウン （ここでは`geographicallocation`）を作成する必要があるプロパティ （フィールド）ごとにノードを作成します。
1. 次のプロパティをノード（ここでは「geographicallocation」）に追加し、「**すべて保存**」をクリックします。
   ...
   <tr>
      <td>name</td>
      <td>文字列</td>
      <td>./extendedproperties/geographicallocation （items ノードで作成したフィールド名と同じ値を維持します）</td>
   </tr>
   <tr>
...
1. プロパティノード（ここでは「geographicallocation」）に、`items` という名前の新しいノードを追加します。 items ノードで、ドロップダウンの値ごとにノードを追加します。 ドロップダウンのデフォルト値として機能するように、最初のノードを空白として追加し、ユーザーがフィールドに値を指定しないようにオプションを追加することをお勧めします。 複数のオプション／ドロップダウンの値を追加するには、次の手順を繰り返します。
   ...
   1. フィールドの名前を`item1,`と入力して、型をnt:unstructuredとして保持し、**OK**&#x200B;をクリックします。
...
1. カスタムタブ（ここでは受信者）でフィールドを作成するには、ノード（ここでは GeographicalLocation）を追加します。 このプロパティは、作成した列に対応します。 フィールドを作成するには、次の手順を実行します（フィールド/ノードをさらに作成するには、次の手順を繰り返します）。
...
1. ノードをオーバーレイのダイアログで、レターに次の値が表示されていることを確認します。 その他のアセットタイプについては、パスは以下の表に示す通りです。
   ...
   **パス：** /libs/fd/cm/ma/gui/content/createasset/createletter/jcr:content/body/items/form/items/letterWizard/items/properties/items/letterproperties/items
   ...
   **ノードタイプを一致させる：**&#x200B;選択済み
   ...
   | レター | /libs/fd/cm/ma/gui/content/createasset/createletter/jcr:content/body/items/form/items/letterWizard/items/properties/items/letterproperties/items |
...
1. childlistpage.jsp ファイル（/apps/fd/cm/ma/gui/components/admin/childpagerenderer/childlistpage/childlistpage.jsp）を開き、次の変更を行います。
   ...
   1. &lt;tr> タグ (&lt;tr &lt;%= attrs.build() %>>): の前に以下を追加します。
      ...
      このコードで、GeoLocation はカスタムのノードまたはフィールドの作成時に name プロパティで設定した値です。 カスタムノード/フィールドの作成時に、./extendedproperties/接頭辞：./extendedproperties/GeoLocationでプロパティの名前を指定しました。 このコードで、接頭辞は不要です。
...
sling:orderBefore プロパティを追加して列の場所を指定する場合は、この手順の手順6.4で指定した対応する&lt;td> タグの順序も更新する必要があります。 例えば、この場合、地理的な場所の &lt;td> タグがバージョン列の &lt;td> タグの前に配置されていることを確認する必要があります。
...
1. 新しく作成した集計フォルダーの下に、ノード cm:resourceを追加します。 cm:resourceの下に、include0という名前のノードを追加します。
   ...
   1. cm:resource フォルダーを右クリックし、**Create** > **Create Node**&#x200B;を選択します。 ノードを作成ダイアログに次の値が表示されていることを確認し、「**OK**」をクリックします。
...
>[!NOTE]
>
>まだ検索できない場合は、インデックス作成の問題が原因の可能性があります。 インデックス再作成を行うには、次のノードに移動し、プロパティ「re-index」の値を true に変更します。
>
>/oak:index/cmLucene&quot;とプロパティの値の変更
...
1. geographicallocationなどのカスタムプロパティノードの下に、タイプ「nt:unstructured」の「granite:rendercondition」という名前のノードを作成します。
