...
2. リモートデスクトップ（RDP）を使用してサーバーにログインし、Windows サービスを使用してAEM Forms サーバーを起動します。 サーバーが動作したら、RDP ウィンドウを閉じるだけです。 代わりに、ユーザーからログオフして適切にログアウトします。これにより、サービスがバックグラウンドで実行され続けている間、セッションが正常に終了します。
