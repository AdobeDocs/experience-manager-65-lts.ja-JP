...
We.Gov デモパッケージには、次のビルトインの架空ユーザーが用意されています。
...
1. **Mobile View**:We.Gov ユーザーは、サイズ変更されたブラウザーでモバイルビューをレプリケートできます。
...
Aya は、携帯電話で短いアダプティブフォームに入力して実施要件を確認するための質問に回答します。
...
1. 次の&#x200B;**動的ロジック**&#x200B;を使用して、**家族状況**&#x200B;ドロップダウンを使用して動的機能を示します。
   ...
   1. **離別**：近親者パネルを表示
   1. **死別**：近親者パネルを表示
   1. **子供の有無**: （はい/いいえ）子の依存パネルを表示するラジオボタン。

      1. 複数の依存する子パネルを追加/削除する（「追加/削除」）ボタン。
...
**このセクション：** Aya は、インタラクティブなコミュニケーションへのリンクが含まれたメールを受信します。この通信には、Aya への給付が要約され、入力すべき申請書のフィールドも含まれています。メールには、給付説明書が PDF で添付され、インタラクティブなコミュニケーションのためのレターへのリンクが付いています（インタラクティブなコミュニケーションと同じテーマやブランディングが使用されます）。
