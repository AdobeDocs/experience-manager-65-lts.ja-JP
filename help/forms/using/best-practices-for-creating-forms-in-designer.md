...
フォームは使いやすくなければアクセスできません。シンプルで使いやすいフォームをデザインすることが重要です。コントロールとフィールドのシンプルなレイアウトと、わかりやすく有意義なキャプションとツールのヒントにより、あらゆるユーザーがフォームを容易に使用できるようになります。
整理され、論理的に配置され、明確でシンプルな手順を提供するフォームをデザインすることは、すべてのユーザーができるだけ簡単にフォームに入力するのに役立ちます。タブの順序やキーボードショートカットなどのナビゲーション機能は、フォーム上のオブジェクトの論理的な順序をサポートする必要があります。
...
スクリーンリーダーのユーザーが認識するラベルは、必ずしも視覚的なキャプションと同じである必要はありません。コントロールの目的をより具体的に説明する必要がある場合もあります。
フォーム内の各フィールドオブジェクトについて、アクセシビリティパレット（図3参照）を使用して、スクリーンリーダーが特定のフォームフィールドを識別するために通知するものを指定できます。
...
* ラジオボタンのグループの目的を示す表示
* 各ラジオボタンの意味のあるラベル
ボタンキャプションを使用してラジオボタンにアクセスできるようにするには：
...
また、フォームコントロールのツールヒントなどの代替テキストに、データソースの値を動的に入力することもできます。例えば、フランス語のオブジェクトに対してカスタムツールヒントを表示できます。
接続するスキーマでは、ツールヒントに対して次の定義を行うことができます。
...
フォームに別の順序が必要で、サブフォームの配置またはグループ化では変更を達成できない場合は、デフォルトのタブ順序を変更できます。デフォルトのタブ順序を変更するには、タブ順序パレットを使用してカスタムタブ順序を作成します。
タブ順序パレット（図12参照）を使用すると、フォーム内のオブジェクトが支援テクノロジーによって読み取られ、ユーザーのTab キーによって移動される順序を調べて変更できます。

![タブ順序パレット](/help/forms/using/assets/image-12.png)

図 12：**タブ順序パレット**

タブの順序パレットでは、フォーム上のタブの順序を別の形式で表示できます。フォーム上のすべてのオブジェクトが番号付きリストとして表示されます。各番号は、タブ フロー内のオブジェクトの位置を表します。
タブの順序パレットを開くには、ウィンドウ/タブの順序を選択します。
...
* WCAG 2.0
   ...
   * 1.4.3 コントラスト （最小）: テキストとテキストの画像の視覚的な表示は、次を除いて、少なくとも4.5:1のコントラスト比を持ちます。（レベル AA）
   * 1.4.6 コントラスト （拡張）: テキストとテキストの画像の視覚的な表示は、次を除き、少なくとも7:1のコントラスト比を持ちます。（レベル AAA）
...
* ヘッダー行
選択したサブフォーム行またはテーブル行にヘッダー行の役割を割り当てます。 ほとんどのスクリーンリーダーは、ボディ行セルの内容を読み上げるときに、まずヘッダー行の対応するセルの内容を特定します。
* Body Row
選択したサブフォームまたはテーブルの行にボディ行の役割を割り当てます。 セルにサブフォームが格納されている場合、スクリーンリーダーは、通常、ヘッダー行の対応するセルの内容を読み上げ、その後サブフォームのフィールドを読み上げます。
* フッター行
フッター行の役割を、選択したサブフォームまたはテーブル行に割り当てます。
* (なし)
...
* 選択したセルからのコンテンツ
ただし、一部のスクリーンリーダーは、両方のヘッダー行からコンテンツを読み取ることができない場合があります。
...
ナビゲーション可能なフォーム構造を提供する最も簡単で効果的な方法は、フォームに構造的な見出しと適切に定義されたリストを使用することです。
また、フォームの上下にナビゲーションボタンを追加するなど、フォームの他の領域にスキップできるメカニズムを提供することもできます。フォームの上部には、「データファイルを開く」、「前のページ」、「次のページ」などのボタンを配置できます。フォームの下部には、「データを保存」、「電子メールデータ」、「ページの先頭に移動」、「印刷」などのボタンを配置することができます。
...
オーディオクリップやビデオクリップなどのオーディオやビデオコンテンツがフォームに組み込まれている場合は、このコンテンツにアクセス可能であることを確認する必要があります。具体的には、フォームに組み込まれたビデオクリップに、聴覚障害のあるユーザー向けのキャプション（字幕とも呼ばれます）と、視覚障害のあるユーザー向けの難しいビデオ説明が含まれていることを確認します。ビデオコンテンツと同期されていないオーディオファイルの場合は、単純なトランスクリプトで十分です。
Flash ベースのメディアの場合は、キャプションの提供について[link](/help/forms/using/best-practices-for-creating-forms-in-designer.md)を参照してください。
