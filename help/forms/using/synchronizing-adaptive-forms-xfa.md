...
1. `sample-xfa-af`：sample-form.xdp ファイルに基づくアダプティブフォーム ただし、このアダプティブフォームにはフィールドは含まれていません。 次の手順で、このアダプティブフォームにコンテンツを追加します。
...
1. メッセージ内の「`Click Here`のラベル」が付いたリンクをクリックします。 XDP ファイルにそのフィールドが存在しなくなったことを示すエラーメッセージが表示されます。
...
[ファイルの取得](assets/sample-xfa-af-sync-1.0.zip)
