...
次のコードの例では、サービスユーザーの役割をプリンシパルに割り当て、そのプリンシパルの役割を印刷し、その後、その役割をプリンシパルから削除します。 このクイックスタートでは、DirectoryManager サービスとAuthorizationManager サービスの2つのサービスが呼び出されます（[役割と権限の管理](/help/forms/developing/users.md#managing-roles-and-permissions)を参照）。
