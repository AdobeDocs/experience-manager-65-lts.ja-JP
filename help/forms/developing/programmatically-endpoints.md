...
* **includeFilePattern**：スキャンおよび取得の対象となるフォルダーとファイルを判別するために監視フォルダーが使用するパターンです。 例えば、この値が `*` であれば、`input*` に一致するすべてのファイルとフォルダーが取得されます。 `input1` や `input2` などといった名前のファイルとフォルダーがこれに該当します。 デフォルト値は `*` です。 この値は、すべてのファイルとフォルダーが対象になることを示しています。 また、ワイルドカードパターンをパターンに追加してファイルパターンを指定することもできます。 監視フォルダーでは、`*.*` や `*.pdf` などのワイルドカードパターンをサポートするよう、正規表現を変更しました。 これらのワイルドカードパターンは、正規表現では対応していません。 この値は必須です。
