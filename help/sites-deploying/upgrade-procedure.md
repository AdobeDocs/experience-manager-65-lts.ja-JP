...
AEM 環境をアップグレードする際は、オーサー環境またはパブリッシュ環境のアップグレード方法の違いを考慮して、オーサーとエンドユーザーの両方のダウンタイムを最小限に抑える必要があります。 このページでは、AEM 6.xのバージョンで現在実行中のAEM トポロジをアップグレードするための概要を説明します。 オーサー層とパブリッシュ層、MongoおよびTarMK ベースのデプロイメントではプロセスが異なるため、各層とマイクロカーネルは別のセクションに記載されています。 デプロイメントを実行するときは、最初にオーサー環境をアップグレードし、成功を確認してから、パブリッシュ環境をアップグレードすることをお勧めします。
...
1. アップグレードされたMongo インスタンスに接続された新しいAEM 6.5 LTS オーサーインスタンスを作成します。
