...
インデックスは、**Oak:index** ノードの下のリポジトリでノードとして設定されます。

インデックスノードのタイプは **oak:QueryIndexDefinition にする必要があります。** とする必要があります。各インデクサーでは、ノードプロパティとして複数の設定オプションを使用できます。 詳しくは、以下の各インデクサータイプに関する設定の詳細説明を参照してください。
...
1. **oak:index** の下にノードを作成します。
1. ノードに **PropertyIndex** という名前を付け、ノードタイプを **oak:QueryIndexDefinition** に設定します。
...
* **propertyNames** プロパティは、インデックスに保存されるプロパティのリストを示します。欠落している場合は、プロパティ名の参照値としてノード名が使用されます。この例では、ノードの一意識別子（UUID）を公開するジョブを持つ **jcr:uuid** プロパティがインデックスに追加されています。
...
1. ノードに **LuceneIndex** という名前を付け、ノードタイプを **oak:QueryIndexDefinition** に設定します。
1. ノードに次のプロパティを追加します。
