---
title: AEM 6.5 LTSでのノードストアとデータストアの設定
...
セグメントノードストアは、AEM 6.5 LTSでのAdobeのTarMK実装の基盤です。 このストアでは、`org.apache.jackrabbit.oak.segment.SegmentNodeStoreService` という PID を設定に使用します。
...
これは、Jackrabbit 2に存在する[FileDataStore](https://jackrabbit.apache.org/api/trunk/org/apache/jackrabbit/core/data/FileDataStore.html)の実装です。 バイナリデータを通常のファイルとしてファイルシステムに保存する方法を提供します。 このストアでは、`org.apache.jackrabbit.oak.plugins.blob.datastore.FileDataStore` という PID を使用します。
...
新しいバージョンの1.60.x S3 コネクタにアップグレードするには、次の手順に従います。

1. AEM インスタンスを停止します。

1. AEM インストールフォルダーの`<aem-install>/crx-quickstart/install`に移動し、そのすべてのサブフォルダーのバックアップを作成します。
1. バックアップの後、次のように、`<aem-install>/crx-quickstart/install/` フォルダーのすべてのフォルダーにあるすべてのjar ファイルを削除して、古いバージョンのS3 コネクタとその依存関係を削除します。
...
1. レプリケーションエージェントの設定に移動し、*https://localhost:4502/etc/replication/agents.author/publish.html*&#x200B;のページを開きます。
...
1. *https://&lt;serveraddress:port>/system/console/jmx*&#x200B;のJMX コンソールに移動します
1. **RepositoryManagementを検索しています。** リポジトリマネージャーのMBeanを見つけたら、それをクリックして使用可能なオプションを表示します。
