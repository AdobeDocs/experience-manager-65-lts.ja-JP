...
>以下はデフォルトで有効になっています。
>
>* オーサーのエージェント：デフォルトエージェント（パブリッシュ）以外の場合は、必ず有効にしてから先に進みます。
>
...
> レプリケーションエージェントの監視[を参照](#monitoring-your-replication-agents)。
...
* [既定のエージェント](#replication-author-to-publish)
オーサーからパブリッシュへのレプリケートに使用します。
...
* 静的エージェント
これは「ノードの静的表現をファイルシステムに保存するエージェント」です。
例えば、デフォルト設定では、コンテンツページとdam アセットは、HTMLまたは適切なアセットフォーマットとして`/tmp`に保存されます。設定については、`Settings`と`Rules`のタブを参照してください。
これは、ページがアプリケーションサーバーから直接要求された場合にコンテンツを表示できるように要求されました。これは特殊なエージェントであり、ほとんどの場合（おそらく）必要ありません。
...
* **HTTP ヘッダー**
  ...
  >[!NOTE]
  >
  >推奨されるデフォルトコンテキスト以外のコンテキストでAEMをインストールした場合は、HTTP ヘッダーにコンテキストを登録する必要があります。例：
...
インストール後、localhost のポート 4503 で稼働するパブリッシュインスタンスへのコンテンツレプリケーションが行えるように、デフォルトエージェントがあらかじめ設定されています。
