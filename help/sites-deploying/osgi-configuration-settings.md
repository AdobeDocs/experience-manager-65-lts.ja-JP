...
[OSGi](https://www.osgi.org/)は、AEMのテクノロジースタックの基本的な要素です。 AEMの複合バンドルとその設定を制御するために使用されます。
...
以下のOSGi設定設定（バンドルに従ってリストされています）は、プロジェクトの実装に関連しています。 リストされているすべての設定を調整する必要はありませんが、AEM の動作を理解するのに役立つ設定もいくつか記載されています。
...
>[!NOTE]
>
>AEM内の特定の機能領域には、さらにバンドルが必要になる場合があります。 このような場合、適切な機能に関連するページに設定の詳細が表示されます。
...
> インスタンスの[ セキュリティ ](/help/sites-administering/security-checklist.md)を確保するには、最初のインストール後にパスワードを変更する必要があります。
...
* **ロガー名**&#x200B;および&#x200B;**ログ形式**&#x200B;を使用して、リクエストとアクセスのロギングの場所と形式を設定します（デフォルト：`request.log`）。 このログファイルは、web チェーンに関連するパフォーマンスの分析や機能のデバッグに不可欠です。 これは [Apache Sling Request Logger](#apacheslingrequestlogger) とペアで使用されます。
...
* **キューのサイズ**。プールが使い果たされた場合のスレッドキューの最大サイズです。
キューを無制限に設定するため、推奨値は`-1`です。制限が設定されている場合、制限を超えると損失が発生する可能性があります。

* これらの設定を変更すると、イベント数が多いシナリオでのパフォーマンスに役立ちます。 たとえば、AEMのDAMやワークフローの使用量が多い場合。
...
* **デフォルトレンディション（** HTML **、** プレーンテキスト **、** JSON **、** XML **など）を有効または無効にします。
JSONを無効にしないでください。**
...
* **インストールフォルダー名regexp**&#x200B;および&#x200B;**インストールフォルダーの最大階層深度** - インストールするリソースを検索する場所と深度を指定します。 ワイルドカードを使用すると（。&#42;/installのように）、適切な一致（例：`/libs/sling/install`と`/libs/cq/core/install`）がすべて検索されます。

* **Search Path**：インストールするリソースを jcrinstall が検索するパスのリストです。そのパスの重み付け係数を示す数値も表示されます。

**Apache Sling キュー設定** ジョブのスケジュールを管理するパラメーターを設定します。

* **再試行間隔**、**最大再試行回数**、**最大並列ジョブ**&#x200B;など。
...
* このような設定は本番環境で役立ちます。例えば、特定のサービスに関するメッセージを個別のログファイルに記録すると、監視を容易化することができます。
...
**Apache Sling Commons MIME Type Service**&#x200B;設定：
...
* **実行パス** – 実行可能なスクリプトを検索するパスを一覧表示します。特定のパスを設定することで、実行できるスクリプトを制限できます。パスが設定されていない場合は、デフォルトが使用され（`/` = ルート）、すべてのスクリプトの実行が可能になります。
設定されたパス値がスラッシュで終わる場合、サブツリー全体が検索されます。このような末尾のスラッシュを使用しない場合、スクリプトは完全一致の場合にのみ実行されます。
...
設定を作成する際には、工場出荷時の設定に変更を加えないでください。 代わりに、次の場所で使用可能な設定マネージャーを使用して、このコンポーネントの工場出荷時の設定を作成します：**https://localhost:4502/system/console/configMgr/**。 このプロキシ設定は、**org.apache.http.proxyconfigurator** で利用できます。
...
**Adobe Granite HTTP Header Authentication Handler** HTTP リクエストの基本認証方式のシステム全体の設定。
...
* **パス**
この認証ハンドラーがアクティブなパス。 このパラメーターを空のままにすると、認証ハンドラーは無効になります。 例えば、/ というパスを指定すると、認証ハンドラーはリポジトリ全体に対して使用されます。

* **サービスランキング**
OSGi Framework サービスのランキング値は、このサービスの呼び出しに使用される順序を示すために使用されます。この値は`int`値で、値が大きいほど優先順位が高くなります。
デフォルト値は`0`です。

* **ヘッダー名**
ユーザーIDを含むことができるヘッダーの名前。

* **Cookie名**
ユーザーIDを含む可能性のあるCookieの名前。

* **パラメーター名**
ユーザーIDを提供する可能性のあるリクエストパラメーターの名前。

* **ユーザーマップ**
選択したユーザーの場合、HTTP リクエストから抽出されたユーザー名は、資格情報オブジェクト内の別のユーザー名に置き換えることができます。 マッピングはここで定義されます。 ユーザー名 `admin` がマップの両側に表示される場合、マッピングは無視されます。 「=」文字を使用する場合は、先頭に「\」を付けてエスケープする必要があります。

* **形式**
ユーザーIDを指定する形式を示します。 使用方法:

   * `Basic`：ユーザー ID が HTTP Basic 認証形式でエンコードされている場合
   * `AsIs`：ユーザー ID がプレーンテキストで指定されている場合や、正規表現が適用されている値をそのまま、または正規表現として使用する必要がある場合

**Day CQ WCM Debug Filter**：ページへのアクセス時に ?debug=layout などのサフィックスを使用できるので、開発時に便利です。 例えば、https://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layoutは、開発者が関心を持つ可能性のあるレイアウト情報を提供します。
...
* オーサーインスタンスでは、このモードは`edit`、`disable,preview`または`analytics`である可能性があります。
その他のモードはサイドキックからアクセスできるか、サフィックス `?wcmmode=disabled`を使用して実稼動環境をエミュレートできます。
...
また、このサービスには、CDN 書き直しの有効化／無効化、CDN 書き直しが実行されるパスの接頭辞、TTL 値、プロトコル（HTTP または HTTPS）など、その他の設定オプションも含まれます。
