...
アップグレードを実行する際には、十分なディスク容量があることを確認してください。
...
以前は手動で実行する必要があった、アップグレード前のメンテナンスタスクの最適化と自動化が進められています。 アップグレード前のメンテナンスの最適化により、これらのタスクをトリガーする統一された方法が可能になり、必要に応じてその結果を検査することができます。
...
1. *https://serveraddress:serverport/system/console/jmx*&#x200B;のJMX コンソールに移動します
...
| runAllPreUpgradeHealthChecks （shutDownOnSuccess） | アクション | アップグレード前のすべてのヘルスチェックを実行し、そのステータスをsling ホームパスのpreUpgradeHCStatus.propertiesという名前のファイルに保存します。 shutDownOnSuccessがtrueに設定されている場合、AEM インスタンスはシャットダウンされますが、アップグレード前のヘルスチェックがすべてOK ステータスである場合に限ります。 プロパティファイルは、今後のアップグレードの前提条件として使用され、アップグレード前のヘルスチェックの実行に失敗した場合は、アップグレードプロセスが停止されます。 アップグレード前のヘルスチェックの結果を無視してアップグレードを起動する場合は、ファイルを削除できます。 |
| detectUsageOfUnavailableAPI （aemVersion） | アクション | 指定したAEM バージョンにアップグレードする際に満たされなくなったすべてのインポート済みパッケージを一覧表示します。 ターゲット AEM バージョンは、パラメーターとして指定する必要があります。 |
