...
MicroKernelは、AEM 6.2以降の永続マネージャーとして機能します。 ニーズに合ったものを選択するには、インスタンスの目的と検討しているデプロイメントタイプによって異なります。
...
>[!NOTE]
>
>この TarMK の例のコールドスタンバイデプロイメントでは、フェイルオーバーサーバーに定期的にレプリケーションが行われるので、プライマリインスタンスとスタンバイインスタンスの両方が別々にライセンスされている必要があります。 ライセンスについて詳しくは、[アドビの一般ライセンス条件](https://www.adobe.com/jp/legal/terms/enterprise-licensing.html)を参照してください。
...
1. 同時ユーザー数（数百人以上）
1. 1 日あたりのアセット取り込みのボリューム（数十万件以上）
1. 1 日あたりのページ編集のボリューム（数十万件以上）（マルチサイトマネージャーやニュースフィードの取り込みなどによる自動化された更新を含む）
