...
>[!CAUTION]
>
>上記のいずれかの実行モード（オーサー、パブリッシュ）を使用する場合、インストール時に使用される値によって、そのインストールの&#x200B;*全期間*&#x200B;の実行モードが定義されます。
>
...
1. [システム プロパティ （`-D`）](#using-a-system-property-in-the-start-script)
...
カスタム実行モードは、クイックスタート起動時に`-r`オプションを使用することで起動することができます。 例えば、次のコマンドを使用して、実行モードがdevに設定されたAEM インスタンスを起動します。 &grave;&grave;
