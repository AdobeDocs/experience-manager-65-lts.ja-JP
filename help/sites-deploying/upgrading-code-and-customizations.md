...
* Apache Felix HTTP SSL Filter Configuration へのアップデート：AEM 6.5 LTS では、`org.apache.felix.http.sslfilter` バンドルがバージョン 1.2.6 から 2.0.2 にアップグレードされました。このアップグレードの一環として、OSGi 設定の PID `org.apache.felix.http.sslfilter.SslFilter` は廃止され、新しい PID `org.apache.felix.http.sslfilter.Configuration` に置き換えられました。 デプロイメントで SSL フィルターを使用する場合は、OSGi 設定マネージャー（`/system/console/configMgr`）を使用して、既存の設定を新しい PID に手動で移行する必要があります。 設定を移行しないと、アップグレード後に SSL フィルターが期待どおりに適用されない場合があります。
