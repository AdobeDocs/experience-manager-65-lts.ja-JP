...
必要なサーブレット APIの最小バージョンはサーブレット 3.1です。 さらに、AEM FormsはJar用のJakarta サーブレット 5をサポートしており、Jakarta サーブレット API 5/6を実装するアプリケーションサーバーにwarをデプロイできます。

| Platform | サポートレベル |
|---|---|
| **クイックスタート ビルトイン サーブレット エンジン （Jetty 11.0.x）** | A：サポート対象 |
| IBM® WebSphere® Application Server Continuous Delivery （LibertyProfile） with Web Profile 24.0.0.7 and IBM® Sumeru open JRE® 17/21 | R：新規契約向けの制限サポート `[1]` |
...
1. Linux® Kernel 5. x および 6. xには、Red Hat® Enterprise Linux®、CentOS、Oracle Linux®、Amazon Linux®などのRed Hat® ディストリビューションの派生製品が含まれています。
1. Adobe Managed Services でサポートされている Linux® ディストリビューション。

   >[!NOTE]
   >
   >Linux ベースのサーバーの場合、AEM Forms アドオンには次のようなランタイム依存関係が必要です。
...
* Microsoft Visual C++ 2019 （VC 14.28以降） 64 ビット版AEM Forms Designerの64 ビットランタイム
