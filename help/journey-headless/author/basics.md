...
コンテンツフラグメントについて詳しく説明する前に、AEMの使用に関する簡単な説明を…介します。ただし、ログインしてシステムを使用しようとする体験に代わるものはありません。
...
ほとんどのシステムと同様に、ログインする必要があります。 作成者には、次の情報が提供されます。
...
以上、AEM ユーザーインターフェイス（UI）について簡単に説明しました。実際に UI を試してみる機会があれば幸いです。 ここからは、実際の関心事である、ヘッドレスのコンテンツフラグメントについて説明します。
...
>[!NOTE]
>
>詳しくは、「バリエーション - コンテンツフラグメントのオーサリング」を参照してください。
...
* **関連コンテンツ**
  ...
  コンテンツフラグメント機能は、AEM ではかなり前のバージョンから利用可のです。 もともとは、ページをオーサリングする際に「従来の」使用に使用できるように…り、このコンテキストではまだ使用されています。 これには、フラグメントに埋め込まれてはいないものの、ページのオーサリング時に作成者が使用できる必要があるアセット（画像など）の関連付けが含まれる場合があります。
