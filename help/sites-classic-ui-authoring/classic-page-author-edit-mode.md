...
* **画像**
  ...
  * **画像**
    コンテンツファインダーから画像をドラッグするか、クリックして参照ウィンドウを開き、画像を読み込むことができます。 画像を読み込んだ後、画像を切り抜いたり、回転したり、削除したりできます。 画像のズームインおよびズームアウトを行うには、画像の下（「OK」および「キャンセル」ボタンの上）のスライドバーを使用します。

  * **トリミング**
    画像の一部を切り抜くことができます。 境界線をドラッグすると、画像が切り抜かれます。
...
* **カルーセル**
ここでは、カルーセルの操作方法を指定します。

  * 再生速度
    次のスライドが表示されるまでの時間（ミリ秒単位）。
  * 遷移時間
    2つのスライド間の移行の時間（ミリ秒単位）。
  * コントロールのスタイル
    プルダウンメニューから、様々なオプションを使用できます。例えば、「前/次ボタン」、「右上スイッチ」などです。

* **リスト**
ここでは、ページをカルーセルに含める方法を指定します。

  * **使用してリストを作成**
    ページリストを作成するには、子ページ、固定リスト、検索または詳細検索の方法がいくつかあります（以下に説明します）。
    どの方法を選択しても、リストに含めるページには、ページに関連付けられた画像がそれぞれ既に含まれている必要があります。カルーセルに表示されるのはこの画像です。そのページのページプロパティの下に画像がない場合は、開始する前に画像をページに関連付ける必要があります。それ以外の場合は、カルーセルに空白ページが表示されます。[&#x200B; ページプロパティの編集](/help/sites-classic-ui-authoring/classic-page-author-edit-page-properties.md)を参照してください。
    ...
    * **検索のオプション**
      ...
      * **で開始**
        開始パスを手動またはセレクターを使用して入力します。

      * **検索クエリ**
        プレーンテキスト検索クエリを入力することもできます。

    * **詳細検索のオプション**

      * **Querybuilder述語**
        Querybuilder述語表記法を使用して検索クエリを入力できます。例えば、「fulltext=Marketing」と入力すると、コンテンツに「Marketing」が含まれているすべてのページがカルーセルに表示されます。
        クエリ式とその他の例について詳しくは、[QueryBuilder API](/help/sites-developing/querybuilder-api.md)を参照してください。
...
* **データ**
  ...
  * **チャートデータ**
    CSV形式を使用してチャートデータを入力します。コンマ区切り値形式では、フィールド区切り文字としてコンマ（&quot;,&quot;）が使用されます。

* **アドバンス**

  * **グラフの種類**
    円グラフ、折れ線グラフ、棒グラフから選択します。
...
* **ダウンロード**
  ...
  * **説明**
    ダウンロードリンクと共に表示される簡単な説明。

  * **ファイル**
    ダウンロード可能なファイルが結果のweb ページに表示されます。 コンテンツファインダーからアセットをドラッグするか、ダウンロード可能にするファイルをアップロードできる領域を選択します。
...
* 検索
  ...
  * **テキストをスペルチェック**
    ...
    誰かが類似の用語を入力すると、このテキストは用語の前に表示されます。
    例えば、`Geometrixxe`と入力すると、システムに「意図していましたか？」と表示されます。Geometrixx」。
...
* 表示する&#x200B;**タグ**
表示するタグが収集される場所。 ページ（すべての子またはすべてのタグを含むページ）から選択します。

* **ページ**
参照するページを選択します。

* **タグにリンクがありません**
表示されるタグがリンクとして機能するかどうかを確認します。
...
テキストコンポーネントを使用すると、[リッチテキストエディター](/help/sites-classic-ui-authoring/classic-page-author-rich-text-editor.md)の機能を備えた WYSIWYG エディターでテキストブロックを入力することができます。 フォントの文字、配置、リンク、リスト、インデントなど、多様なアイコンでテキストの書式を設定できます。
...
* **タイトルとテキスト**
  ...
  * **要素名**
    ...
    フォーム要素の名前。リポジトリ内のデータの保存場所を示します。
...
* **詳細**
  ...
  * **読み込み元パス**
    ...
    フォームフィールドに定義済みの値を読み込むために使用されるノードプロパティのパス。
    このフィールドはオプションで、リポジトリ内のノードへのパスを指定します。このノードにフィールド名と一致するプロパティがある場合、フォーム上の適切なフィールドには、それらのプロパティの値がプリロードされます。一致が存在しない場合、フィールドにはデフォルト値が含まれます。
    **パスの読み込み**&#x200B;を使用すると、必須フィールドの値を含むフォームをプリロードできます。[&#x200B; フォーム値のプリロード &#x200B;](/help/sites-developing/developing-forms.md#preloading-form-values)を参照してください。
  ...
  * **アクションの設定**
    ...
    * **コンテンツを作成**
      ...
      * コンテンツパス
フォームがダンプするコンテンツのコンテンツパス。スラッシュ `/`で終わるパスを入力してください。スラッシュは、各フォームポートについて、指定された場所に新しいノードが作成されることを意味します。例えば、次のようになります。
        `/forms/feedback/`
