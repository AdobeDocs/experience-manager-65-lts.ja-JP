...
リッチテキストエディターは、様々な機能を提供します。これらの[は、個々のコンポーネントの設定](/help/sites-administering/rich-text-editor.md)によって異なります。 この機能は、タッチ操作向けUIとクラシック UIの両方で利用できます。
...
* コピー（Ctrl + C）
* 貼り付け
これは、コンポーネントのデフォルトの貼り付けメカニズム（Ctrl-V）です。標準でインストールされた場合、これは[!UICONTROL Wordから貼り付けるように設定されます]。
