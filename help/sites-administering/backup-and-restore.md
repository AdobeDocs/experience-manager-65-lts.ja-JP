...
この方法は、「ホット」または「オンライン」バックアップとして機能するので、リポジトリの実行中に実行できます。 したがって、バックアップの実行中にリポジトリを使用できます。 このメソッドは、デフォルトのTar ストレージベースのリポジトリインスタンスで機能します。
...
いずれの場合も、バックアップによってリポジトリのイメージ（またはスナップショット）が作成されます。 次に、システムバックアップエージェントは、このイメージを実際に専用バックアップシステム（テープドライブ）に転送するように注意する必要があります。
...
**遅延** リポジトリのパフォーマンスが影響を受けないように、遅延時間をミリ秒単位で指定します。 デフォルトでは、リポジトリバックアップはフルスピードで実行されます。 オンラインバックアップの作成を遅くすることで、他のタスクを遅くしないようにできます。

非常に大きな遅延を使用する場合は、オンラインバックアップに24時間以上かからないようにしてください。もしそうなら、すべてのバイナリが含まれていないかもしれないので、このバックアップを破棄します。
通常、1 ミリ秒遅れるとCPUの使用率が10%になり、10 ミリ秒遅れるとCPUの使用率は3%未満になります。リポジトリーサイズ（MB単位）、ディレイ（ミリ秒単位）を掛け合わせた合計遅延時間を、2で割った値（zip オプションを使用した場合）、または4で割った値（ディレクトリにバックアップする場合）と推定できます。 つまり、200 MBのリポジトリのディレクトリに1 ミリ秒遅延でバックアップすると、バックアップ時間が約50秒増加します。
...
AEM Online Backupは、バックアップするデータと作成するバックアップファイルの整合性を確保するための一連の内部処理で構成されています。 以下に、興味のある方のリストを示します。
...
コンテンツをバックアップおよび復元するには、コンテンツパッケージ形式を使用してコンテンツをバックアップおよび復元するパッケージマネージャーのいずれかを使用できます。 パッケージマネージャーは、パッケージの定義と管理をより柔軟にします。
