...
1. アプリケーションレベルでの制御を組み込みます。 可能なバリエーションの数が多いため、デフォルト設定は実行できません。
...
AEM は `FormChooserServlet` 用の標準インデックスを提供していないので、クエリでフォームセレクターを使用すると、コストの高いリポジトリトラバーサルが発生する可能性があり、通常は AEM インスタンスが過負荷になって最終的に停止します。 フォームセレクターは、クエリに&#x200B;**&ast;.form.&ast;**&#x200B;文字列が含まれていることで検出できます。
...
1. ターゲットインスタンスが既に実行中の場合は、Crypto Bundleinを更新します。
