...
レポートには、さまざまな形式があります。 次のレポートはすべて、次の節で詳しく説明するようにカスタマイズできる列を使用しています。
...
レポートの各列にはドロップダウンメニューがあります。 これは、マウスカーソルが列のタイトルセルの上に移動したときに表示されます。
...
* **編集**&#x200B;ダイアログボックスが開くので、「**データスナップショットがありません**」を選択して、必要になるまで収集を停止します。
...
>[!NOTE]
>
>事前設定済みのレポートはパフォーマンスを集中的に消費しませんが、本番環境では毎日のスナップショットを使用することをお勧めします。 可能であれば、Web サイト上でのアクティビティが少ない時間帯に、これらの毎日のスナップショットを実行します。 これは **Day CQ Reporting Configuration**&#x200B;の `Daily snapshots (repconf.hourofday)` パラメータで定義できます。 OSGi を設定する方法の詳細に関しては [OSGi 設定](/help/sites-deploying/configuring-osgi.md)を参照してください。
...
チャートをより長い期間にわたってクリーンに保つために、設定できる制限があります。 標準レポートでは、次の項目を使用します。
...
カスタマイズされたレポートには、すべてのシリーズの&#x200B;**合計**&#x200B;値も表示できます。 これは系列（凡例の水平線とエントリ）として表示されます。
...
* **スナップショット**
スナップショットを作成する頻度（毎日、毎時間、またはまったく作成しない場合）を定義できます。
