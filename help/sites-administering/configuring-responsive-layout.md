...
これにより、コンポーネントのサイズをインタラクティブに変更して、デバイスやウィンドウのサイズに合わせてレイアウトを並べ替えるレスポンシブ web サイトを作成および編集できます。 その後、ユーザーはエミュレーターを使用してコンテンツがどのようにレンダリングされるかを確認できます。
...
1. 「ブレークポイント」ノードでは、任意の数のブレークポイントを作成できます。 各定義は、次のプロパティを持つ1つのノードです。
...
アダプティブ画像メカニズムは、ウィンドウの現在のサイズに対する正しい画像の選択を制御するスクリプトを介して使用できます。 これは、DOMの準備が整った後、または専用のイベントを受信した後にアクティブになります。 現在、ユーザーのアクションの結果を適切に反映するには、ページを更新する必要があります。
...
作成者がコンテンツページにさらにレスポンシブグリッドを追加できるようにするには、ページのレイアウトコンテナコンポーネントを有効にする必要があります。 次のいずれかを使用して行うことができます。
...
1. **オーサー環境**
   ...
   これには、[&#x200B; デザインモード &#x200B;](/help/sites-authoring/default-components-designmode.md)を使用してから、必要なコンテナのデザインダイアログを開きます。 ここでは、配置とサイズ調整に使用できる列数を指定できます。 デフォルトは12です。
...
ネストされたレスポンシブグリッドの使用を避けられない場合は、次の点を確認してください。

* すべてのコンテナ（コンテナ、タブ、アコーディオンなど） プロパティ `layout = responsiveGrid`があります。
