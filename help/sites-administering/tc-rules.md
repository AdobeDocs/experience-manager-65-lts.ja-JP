---
title: 翻訳するコンテンツの特定
description: Adobe Experience Manager で翻訳が必要なコンテンツを特定する方法について説明します。
...
**updateDestinationLanguage** この属性は、テキストを持たず、言語コード（jcr など）を持つプロパティに対して使用され :language す。 ユーザーはテキストを翻訳するのではなく、言語ロケールをソースから宛先に翻訳します。そのようなプロパティは、翻訳用に送信されません。
