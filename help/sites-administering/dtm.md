...
<table>
 <tbody>
  ...
  <tr>
   <td>共有暗号鍵</td>
   <td><p>（オプション）ダウンロードの復号に使用する共有暗号鍵。 この値は、Dynamic Tag Management のライブラリダウンロードページの「共有暗号鍵」フィールドから取得します。</p> <p><strong>メモ：</strong>AEM がダウンロードしたライブラリを復号できるよう、AEM がインストールされているコンピューター上に OpenSSL ライブラリをインストールしておく必要があります。</p> </td>
  </tr>
  <tr>
  ...
  <tr>
   <td>スケジュール式</td>
   <td>（ポーリングインポーターを有効にするが選択されている場合に表示され、必須になります）。 Dynamic Tag管理ライブラリをダウンロードするタイミングを制御するcron式。</td>
  </tr>
 </tbody>
</table>
