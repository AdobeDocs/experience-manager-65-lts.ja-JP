...
| いいえ | 不可 | はい | 有効な権限評価によってアクセス権が付与されている場合、認証されたユーザーにも、認証されていないユーザーにも、CUG ポリシーのマークが付いたサブツリーのみ表示されます。 認証されていないユーザーも同じように処理され、ログインにリダイレクトされません。 |
...
特定のページの下にあるjcr:content ノードにのみCUG ポリシーを配置すると、特定のページのコンテンツ s.strへのアクセスのみが制限されますが、兄弟ページや子ページには適用されません。 これは有効なユースケースであり、きめ細かなアクセスコンテンツを適用可能なリポジトリエディターによって実現できます。 ただし、jcr:content ノードにcq:cugEnabled プロパティを配置すると、内部でページノードに再マッピングされる以前の実装とは対照的です。 こうしたマッピングは今後は行われません。
...
前述のように、CUG アクセス制御ポリシーは常にコンテンツに保存されますが、これらのポリシーから生じる効果的な権限の評価は、Apache Jackrabbit Oak **CUG Configuration**&#x200B;のシステムコンソールで&#x200B;**CUG Evaluation Enabled**&#x200B;がオンになっている場合にのみ適用されます。 デフォルトでは、「公開」実行モードでのみ有効になっています。
...
特定のページの下にあるjcr:content ノードにのみCUG ポリシーを配置すると、コンテンツへのアクセスのみが制限されます。 ただし、そのページノード自体や子ページには影響しません。

場合によっては、こうしたケースも有効なシナリオであり、任意のノードに mixin を配置できるリポジトリエディターを使用して実現できます。 ただし、この動作は、jcr:content ノード上にcq:cugEnabledまたはcq:cugLoginPage プロパティを配置すると、最終的にページノードに内部的に再マッピングされる以前の実装とは対照的です。 こうしたマッピングは今後は行われません。

#### 設定済みのサポートパス {#configured-supported-paths}

`granite:AuthenticationRequired` Mixin タイプとgranite:loginPath プロパティの両方は、**Adobe Granite Authentication Requirement and Login Path Handler**&#x200B;で指定された&#x200B;**サポートされているパス**&#x200B;設定オプションのセットで定義された範囲内でのみ尊重されます。 これらのパスが指定されていないと、認証要件機能は完全に無効になります。 その場合、mixin タイプやプロパティを追加しても、特定の JCR ノードに設定しても、どちらも有効になりません。
