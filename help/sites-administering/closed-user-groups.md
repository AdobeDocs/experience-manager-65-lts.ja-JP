...
特定のページの下にあるjcr:content ノードにのみCUG ポリシーを配置すると、特定のページのコンテンツ s.strへのアクセスのみが制限されますが、兄弟ページや子ページには適用されません。 これは有効なユースケースであり、きめ細かなアクセスコンテンツを適用可能なリポジトリエディターによって実現できます。ただし、jcr:cugEnabled ノードにcq:content プロパティを配置すると、内部でページノードに再マッピングされる以前の実装とは対照的です。 こうしたマッピングは今後は行われません。
...
場合によっては、こうしたケースも有効なシナリオであり、任意のノードに mixin を配置できるリポジトリエディターを使用して実現できます。ただし、この動作は、jcr:cugEnabled ノード上にcq:cugLoginPageまたはcq:content プロパティを配置すると、最終的にページノードに内部的に再マッピングされる以前の実装とは対照的です。 こうしたマッピングは今後は行われません。
