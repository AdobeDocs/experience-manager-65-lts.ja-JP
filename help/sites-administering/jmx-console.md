...
コンソールコントロールの使用方法について詳しくは、[JMX コンソールの使用](#using-the-jmx-console)を参照してください。 JMXについて詳しくは、Oracle web サイトの[Java Management Extensions （JMX） Technology](https://www.oracle.com/technetwork/java/javase/tech/javamanagement-140525.html) ページを参照してください。
...
>[!NOTE]
>
>その他のワークフロー管理ツールおよびワークフローインスタンスに考えられるステータスの説明については、[ワークフローコンソール](/help/sites-administering/workflows-administering.md)を参照してください。
...
**terminateFailedInstances** 失敗したワークフローインスタンスを終了します。 失敗したインスタンスをすべて終了するか、特定のモデルの失敗したインスタンスのみを終了するかを指定できます。 オプションで、終了後にインスタンスを再起動できます。 また、操作をテストして、実際に操作を行わずに結果を確認することもできます。
...
**restartStaleWorkflows** 古くなったワークフローインスタンスを再起動します 古くなったインスタンスをすべて再起動するか、特定のモデルの古くなったインスタンスのみを再起動するかを指定できます。 また、操作をテストして、実際に操作を行わずに結果を確認することもできます。
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>ノードとノードのプロパティが同じ名前を持つかどうかを示します。 trueは同じ名前がサポートされていることを示し、falseはサポートされていないことを示します。 </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true はリポジトリ変更の非同期の監視がサポートされていることを示します。 非同期監視のサポートにより、アプリケーションは、発生した各変更に関する通知を受信して応答することができます。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、リポジトリでノード定義にプライマリ項目を子として含めることができることを示します。 プライマリ項目は、項目名がわからなくとも、API を使用してアクセスできます。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true はリポジトリによって API を使用した書き込みアクセスが提供されることを示します。 false は読み取り専用アクセスを示します。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は複数のバイナリプロパティを持つノードタイプの登録をリポジトリがサポートしていることを示します。 false は 1 つのノードタイプに対して単一のバイナリプロパティがサポートされていることを示します。</td>
  </tr>
  <tr>
   <td>option.access.control.supported</td>
   <td>true は、ノードアクセス用のユーザーの権限を設定および決定するために、リポジトリがアクセス制御をサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>option.baselines.supported</td>
   <td>true は、設定とベースラインの両方をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>option.shareable.nodes.supported</td>
   <td>true は、共有可能なノードの作成をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>crx.cluster.id</td>
   <td>リポジトリクラスターの識別子です。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、フルテキスト検索をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>node.type.management.inheritance</td>
   <td><p>ノードタイプの継承に関するリポジトリサポートのレベルを示します。 次の値を指定できます。</p> <p>node.type.management.inheritance.minimal：プライマリノードタイプの登録は、nt:base のみをスーパータイプとして持つものに限られます。 mixin ノードタイプの登録は、スーパータイプがないものに限られます。</p> <p>node.type.management.inheritance.single：プライマリノードタイプの登録は、1つのスーパータイプを持つノードタイプに限定されます。 Mixin ノードタイプの登録は、最大で1つのスーパータイプを持つものに制限されます。</p> <p><br /> node.type.management.inheritance.multiple: プライマリノードタイプは、1つ以上のスーパータイプに登録できます。 Mixin ノードタイプは、0個以上のスーパータイプで登録できます。</p> </td>
  </tr>
  <tr>
   <td>crx.cluster.preferredMaster</td>
   <td>true は、このクラスターノードがクラスターのマスターとして優先的に機能することを示します。</td>
  </tr>
  <tr>
   <td>option.transactions.supported</td>
   <td>true は、トランザクションをリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>jcr.repository.vendor.url</td>
   <td>リポジトリベンダーの URL です。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>登録済みのノードタイプが指定できるプロパティタイプを表すjavax.jcr.PropertyType定数の配列。 長さが0の配列は、登録されたノードタイプがプロパティ定義を指定できないことを示します。 プロパティタイプは、STRING、URI、BOOLEAN、LONG、DOUBLE、DECIMAL、BINARY、DATE、NAME、PATH、WEAKREFERENCE、REFERENCE および UNDEFINED（サポートされている場合）です。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>リポジトリベンダーの名前です。</td>
  </tr>
  <tr>
   <td>query.joins</td>
   <td><p>クエリ内の結合のサポートレベルです。 次の値を指定できます。</p>
    <ul>
     <li>query.joins.none：結合がサポートされていません。 クエリでは 1 つのセレクターを使用できます。</li>
     <li>query.joins.inner：内部結合がサポートされています。</li>
     <li>query.joins.inner.outer：内部結合と外部結合がサポートされています。</li>
    </ul> </td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、XPath 1.0 クエリ言語をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>query.jcrpath</td>
   <td> </td>
  ...
  <tr>
   ...
   <td>true は、XML コードのコンテンツとしての読み込みをリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>node.type.management.same.name.siblings.supported</td>
   <td>true は、同じ名前の兄弟ノード（同じ親を持つノード）をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、ノード作成時におけるそのノードの子項目（ノードまたはプロパティ）の自動作成をリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、リポジトリがノードのロックをサポートしていることを示します。 ロックにより、ユーザーは他のユーザーが変更を加えることを一時的に防ぐことができます。</td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>true は、0 個以上の値を持つことができるノードプロパティをリポジトリがサポートしていることを示します。</td>
  </tr>
  <tr>
   <td>option.retention.supported</td>
   <td>true は、リポジトリが、コンテンツに保持ポリシーを適用するための外部保持管理アプリケーションの使用をサポートし、保持と解放をサポートすることを示します。</td>
  </tr>
  <tr>
...
* ValuePerWeek：過去3年間の1週間あたりの測定値。 読み取り専用。
...
* **タイプ：**&#x200B;エージェント
* **名前：**&#x200B;値なし
* **プロパティ：** {id="*Name*"}。*Name*&#x200B;はエージェント名プロパティの値です。
...
OSGi サービスでデプロイされたMBeanは、サービス属性と操作をコンソールに公開します。 MBeanは、公開される属性と操作、および属性が読み取り専用か読み取り書き込みかを決定します。

JMX コンソールのメインページには、サービスのテーブルが含まれています。 テーブルの各行は、MBeanで公開されるサービスを表します。
