...
<table>
 <tbody>
  ...
  <tr>
   <td>分析 </td>
   <td>Adobe Analyticsを利用したデータを表示する際に、サイトブラウザーで検索/フィルター機能を使用できます。 Analytics検索フィルターは、マッピングされたカスタマイズされたAnalytics列に一致するように読み込まれます。</td>
   <td>
    <ul>
     <li>フィールドラベル</li>
     <li>説明</li>
...
* **オプション ノード パス**
**オプションパス**&#x200B;と実質的に同じですが、これは共通の述語フィールドにあり、もう1つはアセットに固有です。

* **単一選択**
オンにすると、オプションはチェックボックスとしてレンダリングされ、選択は1つのみになります。 チェックボックスを誤って選択した場合は、選択を解除できます。

* **パブリッシュおよびライブコピープロパティ名**
サイト固有の述語の「公開」チェックボックスと「ライブコピー」チェックボックスのラベル。

* 「**設定**」タブのフィールドラベルの&amp;ast；は、フィールドが必須であり、空白のままにするとエラーメッセージが表示されることを意味します。
...
<table>
 <tbody>
  ...
  <tr>
   <td>プレビュー</td>
   <td><code>/var/dam/content</code> ノードの読み取り、書き込み、削除の権限。<br /> <code>/apps</code> ノードの読み取り、書き込み権限。</td>
  </tr>
 </tbody>
</table>
