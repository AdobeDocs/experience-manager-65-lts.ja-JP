...
>[!INFO]
>
>このドキュメントでは、Adobe Campaign StandardをAEM 6.5と統合する方法について詳しく説明します。 Campaignのその他の統合機能については、[AEM 6.5とAdobe Campaignの統合](campaign.md)を参照してください。

## 統合手順 {#integration-steps}

AEM と Adobe Campaign Standard 間の統合を設定するには、両方のソリューションでいくつかの手順が必要です。

1. [Campaignでの`aemserver` ユーザーの設定](#aemserver-user)
1. [Campaignで`AEMResourceTypeFilter`を確認します](#resource-type-filter)
...
1. [AEMでの`campaign-remote` ユーザーの設定](#campaign-remote-user)
...
1. 設定を編集するための新しいウィンドウとダイアログが開きます。 必要な情報を入力します。
   ...
   * **ユーザー名** – これは、前の手順で設定したAdobe Campaignの`aemserver` ユーザー[です。](#aemserver-user) デフォルトでは `aemserver` です。
