...
1. AEM 内で呼び出すアクションを準備します。 この場合、**ページを作成** ウィザードの最後まで進んでいますが、**作成**&#x200B;をまだクリックしていません。
...
1. cURL コマンドをテキストエディターにコピーし、`-H`で始まるコマンドからすべてのヘッダーを削除し（下の画像で青で強調表示）、`-u <user>:<password>`などの適切な認証パラメーターを追加します。
