...
個々のAEM [&#x200B; アクション &#x200B;](security.md#actions)のチェックボックスをオンまたはオフにすることで、特定のユーザーに対して付与/拒否された権限を変更できます。 チェックマークは、アクションが許可されていることを示します。 アクションが拒否されたことを示すチェックマークはありません。
...
>
>後方互換性の理由から、アクションのテストでは、**jcr:content**&#x200B;を定義するノードの特別な処理は考慮されません。
...
権限が親ノードから継承されていないが、少なくとも1つのローカルエントリがある場合、次の記号がチェックボックスに追加されます。 ローカルエントリは、CRX 2.2 インターフェイスで作成されるものです（現在、ワイルドカード ACLはCRXでのみ作成できます）。
...
| *ポジティブに* | 常に「許可」ステートメントを使用してグループの権限を指定します（可能な限り）。 「拒否」ステートメントの使用は避けてください。 グループは順番に評価されます。この順序はユーザーごとに異なる方法で定義される可能性があります。 つまり、ステートメントが実装および評価される順序は、ほとんど制御できない可能性があります。 「許可」ステートメントのみを使用する場合、順序は関係ありません。 |
...
| 権限 | ユーザーまたはグループに権限を割り当てることができます。 次の権限を制御できます。<ul><li>特定のページ／ノードに関連する権限。 [権限の設定](#setting-permissions)を参照してください。 </li><li>ページの作成と削除、および階層の変更に関連する権限。 ??? 階層の変更など、[権限を割り当てることができます](#settingprivileges)。これにより、ページを作成および削除できます。</li><li>（通常は作成者から公開への）パスに従った[レプリケーション権限](#setting-replication-privileges)に関連する権限。</li></ul> |
...
1. **セキュリティ** コンソールで、**ユーザーを非表示**&#x200B;または&#x200B;**グループを非表示**&#x200B;をクリックします。 選択したボタンがハイライト表示されます。
...
1. ユーザーまたはグループのどちらを作成するかに従って、必要な詳細を入力します。
   ...
   * 「**ユーザーを作成」を選択すると、**&#x200B;はログイン ID、氏名、電子メールアドレス、パスワードを入力します。 デフォルトでは、AEMは姓の最初の文字に基づいてパスを作成しますが、別のパスを選択することもできます。
...
>[!NOTE]
>
>ユーザーのパスは、ユーザープロパティの下部に表示されます。 変更できません。
...
1. 「**パスワードを設定**」をクリックします。 パスワードを設定ウィンドウが開き、パスワードを変更できます。
...
権限を使用すると、ユーザーは特定のパスのリソースに対して特定のアクションを実行できます。 また、ページを作成または削除する機能も含まれます。
...
1. 選択したパスの&#x200B;**レプリケーション**&#x200B;列で、そのユーザーまたはグループのレプリケーション権限を追加するチェックボックスを選択するか、チェックボックスをオフにしてレプリケーション権限を削除します。 AEMでは、まだ保存されていない変更を行った箇所に赤い三角形が表示されます。
...
階層の変更権限は JCR 権限の組み合わせによってカバーされます。 レプリケーション権限は&#x200B;**crx:replicate**&#x200B;という名前で、jcr リポジトリ上の他の権限と一緒に保存または評価されます。 ただし、この権限は JCR レベルでは適用されません。
...
新しい（カスタム）権限の登録は、リポジトリレベルで付与する必要がある組み込み権限によって保護されます。 JCRで、「null」をac mgt apiの「absPath」パラメーターとして渡します。詳しくは、jsr 333を参照してください。 デフォルトでは、**admin** と administrators のすべてのメンバーにこの権限が付与されています。
