...
1. 「**ターゲティング**」をクリックします。ページの上部に「ターゲット設定」オプションが表示されます。
...
[ターゲット設定プロセス](/help/sites-authoring/content-targeting-touch.md#the-targeting-process-create-target-and-goals-settings)の目標と設定手順では、ブランドアクティビティの動作を設定します。アクティビティの開始と終了のタイミングおよびアクティビティの優先度を指定します。また、目標もトラックします。特に、アクティビティで何を測定するかを決定できます。
...
>[!NOTE]
>
A/B テストを使用してシミュレーションを行った場合は、トラフィックの割合に基づいてエクスペリエンスが表示されます。この制御は Adobe Target が行いますが、作成者にとって予期せぬ結果になることがあります。（_author アクティビティは、シミュレーション中に再評価を可能にする特定の設定と同期されます。） 作成者がトラフィック設定に基づいて他のエクスペリエンスを表示するには、更新が必要になる場合があります。
