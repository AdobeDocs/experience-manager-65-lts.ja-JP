...
description: Adobe Experience Manager 6.5 LTSによる、パーソナライズされたコンテンツの制作方法をご紹介します。
...
>[!NOTE]
>
>ターゲティングモードと従来のAEM ターゲティングコンポーネントでは、ContextHubに基づいてAEM Target統合を使用します。 したがって、AEP Web SDKのみに依存しているページでは、従来のAEM ターゲティングコンポーネントはレンダリングされません。 [詳しくは、開発者ドキュメントを参照してください。](/help/sites-developing/target.md)
...
例えば、あるアクティビティが「30 歳以上の女性」と「30 歳未満の女性」という 2 つの異なるオーディエンス用のエクスペリエンスを定義するものとします。 We.Retail web サイトの女性向けページでは、エクスペリエンスごとに異なる商品が表示されます。
...
AEM は、ページリクエストの処理や、表示コンテンツの判断を行うビルトインのターゲティングエンジンを備えています。 AEM ターゲティングエンジンを使用する場合、エクスペリエンスのオーディエンス定義には、AEM で作成されるセグメントのみを使用できます。
...
* Adobe Target エンジンを使用するアクティビティは、[Targetに同期されます。](/help/sites-authoring/activitylib.md#synchronizing-activities-with-adobe-target)

このエンジンは、[Adobe Targetと統合されている場合に使用できます。](/help/sites-administering/opt-in.md)
