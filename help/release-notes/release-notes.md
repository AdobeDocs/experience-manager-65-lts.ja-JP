...
* FORMS-23971：フォームデータモデル（FDM）エディターの「Data Source / Enter Keyword」機能で問題が発生しました。 これにより、データソースを検索して選択する機能に影響が及びます。

* FORMS-23754：モバイルデバイスでは、アダプティブFormsのテーブルコンポーネントが上部に非表示のヘッダーを表示し、スクリーンリーダーがコンテンツを誤って読み上げる原因になりました。 この影響を受けたユーザーは、ナビゲーションをスクリーンリーダーに依存しています。

* FORMS-23632:Granite:InternalAreaとしてフラグ付けされたリソースタイプを参照するアダプティブ Formsに基づくコアコンポーネントベースの問題が発生し、オンプレミス Forms アドオンのいくつかのGranite コンポーネントの機能に影響を与えました。

* FORMS-23457:AEM 6.5 LTS SP1へのアップグレード後にフォーム送信が失敗する。 com.adobe.cq.social.commons.CollabUtilが見つからず、JSPのコンパイル エラーとメールアクションのエラーが発生しました。

* FORMS-23426：基盤コンポーネント ベースのアダプティブ Formsで、Captchaが正しく変換されない問題が発生しました。 これは、英語圏のユーザー以外のユーザーがフォームを正確に入力する能力に影響を与えました。

* FORMS-22633: SAXParseException: &quot;Content is not allowed in prolog&quot; （HTTP 500）が発生し、フォーム送信エラーが発生しました。 この問題は、事前入力データ XMLのnull値が原因で発生し、サーバーサイド XMLの解析が失敗しました。

* FORMS-22101：フォームのタブナビゲーションマークアップが無効なため、Adaptive FormsでWeb Content Accessibility Guidelines （WCAG）監査が失敗しました。リスト以外の要素は、リスト項目のみが許可されるリストのダイレクト子としてレンダリングされました。 これにより、法的または社内のコンプライアンス要件を満たす必要があるアクセシビリティバリデーターや影響を受ける組織をフォームが渡すことができませんでした。

* FORMS-21989：空のフォームフィールドがフォーム要素としてタグ付けされていない場合、レコードのドキュメント（DoR）/送信PDFでアクセシビリティの問題が発生しました。 これにより、スクリーンリーダーが困難になり、ユーザーが効果的にフォームを操作して記入する能力に影響を与えました。

* FORMS-21925: フォームの読み込み中に、サブパネル内のコンポーネントの脚注が表示されない問題が発生しました。 これは、脚注が付いた項目がページの最後のコンポーネントであった場合に発生しました。

* FORMS-21814:AEM Forms エディターでコンポーネントを選択する際に問題が発生しました。 タブ間を移動して最初のタブに戻ると、一部のコンテナが選択不可になり、簡単に識別および操作できなくなります。

* FORMS-20679:Adaptive Forms ダッシュボードでセキュリティ上の脆弱性が発生しました。 具体的には、startpointcontrol.js ファイルでクロスサイトスクリプティング（XSS）の問題が特定されました。これにより、悪意のあるスクリプトが実行される可能性があります。

* FORMS-24687: JBoss EAP 8上のAEM Forms 6.5 LTS クラスターのデプロイメントでは、domain/configuration/domain_oracle.xml、domain_mysql.xmlおよびdomain_mssql.xml ファイルに、無効なXMLを引き起こし、Domain Controllerが起動しない重複した`<security>` タグが含まれなくなりました。

* FORMS-24689: ターンキーモードで、新しいインストールとアップグレード時にデータベースポートの更新が正しく適用されるようになりました。 新規インストールモードでは、使用可能なすべてのポートから選択でき、アップグレードモードでは、lc_turnkey.xmlで更新されたデータベースポートがアップグレードプロセス中に正しく参照されます。

* FORMS-24688: LinuxでJBoss EAP 8.0を設定する際に、Windowsで変更されたシェルスクリプトが、CRLF行の終了により`/bin/sh^M: bad interpreter or $'\r': command not found` エラーを引き起こさなくなりました。
...
* **FORMS-24690:** Configuration Managerでは、AEM Forms 6.5 LTS JEE ターンキーカスタムモードでブートストラップ中にデータベースの初期化が失敗します。モジュールが選択されていないか、一部のコンポーネントのみが選択されている場合です。 失敗は、依存関係が欠落しているため（xalan-2.7.2.jar）、エラーが発生します。 adobe-livecycle-jboss.ear\libにJAR ファイルを追加すると、問題が解決します。

* **FORMS-24894:** JBoss EAP 8で実行されているForms JEE LTS デプロイメントで、Reader Extensions UIが内部サーバーエラーで失敗する場合があります。

* **FORMS-24892:** JBossで実行されているForms JEE LTSで、メール関連の機能が失敗する可能性があります。 電子メール機能を使用しようとすると、サーバーが`Error IMAPProvider not a subtype`と同様のエラーをログに記録する可能性があります。

* **FORMS-24741:** Linux プラットフォームでは、Forms JEE LTSでは、Configuration Managerを実行する前に`OSFileSetIntendedFor`の`LFS_Foundation.properties` プロパティを正しく設定する必要があります。 更新されない場合、設定がLinux向けに適切にカスタマイズされない可能性があり、ランタイムまたはデプロイメントの問題が発生する可能性があります。 この問題を解決するには、インストーラーを実行した後、Configuration Managerを実行する前に、`configurationManager/config/solcomp/`に移動し、`LFS_Foundation.properties`を開いて`OSFileSetIntendedFor=Linux`を設定し、ファイルを保存してからConfiguration Managerを実行します。
