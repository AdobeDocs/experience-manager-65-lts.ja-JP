...
description: Adobe Experience Manager 6.5 LTS、Service Pack 2の最新のリリース情報を確認します。
...
**AEM Forms 6.5 LTS SP2**&#x200B;に含まれるもの

* JBoss® EAP 8.0でのRDBMKのサポート  が追加されました。

* ビジュアルルールエディターでのユーザーエクスペリエンスが向上しました。 このアップデートには次が含まれます。

   * 保存後に概要ビューを自動的に再読み込みして、更新されたルールステータスを表示

   * 「追加」/「削除」ボタンを表示し、非表示にするのではなく切り替えることができる

   * ルールの保存操作が失敗した場合に明確なフィードバックを提供する（FORMS-21261）

* AEM Formsの従来の拡張可能マークアップ言語（XML）書き出しモードを切り替えるランタイムアプリケーションプログラミングインターフェイス（API）を追加し、`Dcom.adobe.fd.forms.export.legacy` パラメーターを置き換えました。 この機能強化により、ユーザーは書き出しモードをより効率的に切り替えることができ、ワークフローの柔軟性が向上します。 （FORMS-23115）

* アダプティブ Formsの名前空間タグ付きJavaScript Object Notation （JSON）のサポートを追加しました。 この機能強化により、ユーザーはJSON データ構造をより効果的に処理できるようになり、データ統合と処理機能が向上します。 （FORMS-22519）

* ルールエディターに、「レコードのドキュメントをダウンロード（DoR）/フォーム送信」ボタンを標準の（OOTB）ボタンとして追加しました。 この機能強化により、カスタムコードを記述することなくdownloadDoR関数を使用できるようになり、使いやすさと効率性が向上しました。 （FORMS-21263）

* アダプティブ Formsの名前空間タグ付きJavaScript Object Notation （JSON）のサポートを追加しました。 この機能強化により、ユーザーはフォームにより正確かつ効率的に事前入力できるようになり、データ統合が強化され、手作業による入力エラーが減りました。 （FORMS-10883）

<!-- UPDATE THE EACH RELEASE -->

## 6.5 LTS、サービスパック 2で修正された問題 {#fixed-issues}
...
* ティーザーモーダルヘッダーは、キーボードユーザーがダイアログボックスを移動するのをブロックしなくなりました。 キーボードコントロールで操作をピックアップ、移動、ドロップできるようになり、スクリーンリーダーの使いやすさと全体的な操作性が向上しました。 （SITES-25226）
...
* リンク選択ダイアログボックスで、プログラマティックラベルと最終レベルのツリー項目の画面上のラベルが整列するようになりました。 矢印キーによるナビゲーションは、各項目に対して信頼性の高いスクリーンリーダーのアナウンスをトリガーし、誤解を招くラベル出力を排除します。 （SITES-24710）
...
* ページの更新中に不規則な動作を停止するように、Sites オーサリングインターフェイスのコンポーネント処理を調整しました。 この欠陥により、編集者の予測不可能な回答が生じ、日常的なコンテンツの変更が妨げられ、ワークフロー効率が低下しました。 このアップデートにより、エディターロジックが期待されるインタラクションパターンに合わせて調整され、オーサリングアクティビティ中に信頼できるパフォーマンスが提供されます。 （SITES-35078）重要
...
* リファクタリング後も機能トグルを無効のままにすると、コンテンツフラグメントのバリエーションのタグが消えてしまいます。 この修正により、トグルがオフのままでもバリエーションのタグのサポートが復元されます。 作成者は、コンテンツフラグメントエディターでバリエーションタグを再度追加して表示できます。 （SITES-38682）重要
* 編集済みコンテンツフラグメントエディターから作成者が戻った後、Assets コンソールリストから編集されたコンテンツフラグメントが消えました。 ブラウザーのキャッシュで古いリストが返され、更新されたフラグメントは手作業による更新まで非表示にされました。 この修正により、エディターのリターンパスにキャッシュコントロール処理が追加され、リストが正しくリロードされ、編集されたフラグメントが表示されたままになります。 （SITES-35374）重要
...
* フィルター実行エラーによるデプロイメントエラーを停止するために、GraphQL クエリの検証処理を調整しました。 この欠陥は、アプリケーションの起動時に例外を生成し、影響を受ける環境での正常なロールアウトをブロックしました。 このリビジョンは、一貫した検証動作を保証し、ランタイムクエリ検証の中断なしでスムーズなデプロイメントを可能にします。 （SITES-34301）重要
...
* リファクタリングで機能が無効な切り替えスイッチに関連付けられたときに、ネストされたコンテンツフラグメントモデルが機能しなくなった。 この修正により、切り替えの変更を必要とせずに、ネストされたモデルのサポートが復元されます。 作成者は、モデルエディターでネストされたモデルを再度作成して使用できます。 （SITES-38681）重要
...
AEM ヘッドレスでは、メインラインビルドとの依存関係やバンドルバージョンの競合を避けるために、専用のリリースブランチが必要でした。 このアップデートでは、`release/6.5lts`個のヘッドレスブランチが追加され、依存関係セットとバンドルバージョンが調整されます。 Jenkinsは、バージョンの競合なしでヘッドレスコードベースをクリーンに構築するようになりました。 （SITES-36585）
...
* 標準ページ編集ワークフローを中断する一貫性のない動作を解決するためのSites オーサリングエクスペリエンスの変更。 作成者は、コンポーネントのインタラクション中に予期しない結果が発生し、コンテンツの更新が妨げられ、信頼性が低下しました。 この変更により、安定したエディター動作が復元され、影響を受けるシナリオ間でオーサリングアクションが一貫して実行されます。 （SITES-35162）重要
...
* ユニバーサルエディターを開くコマンドのデフォルト設定は、開発環境とステージング環境のプレビューモードです。 このコマンドは`preview=true`を追加します。これにより、作成者チェックとプレビューコンテキストの整合性が維持され、誤って実稼動環境が開くことを防ぐことができます。 （SITES-33839）
...
* フォームデータモデル （FDM） エディター`Data Source / Enter Keyword`機能に問題が発生しました。 この問題は、データソースを検索して選択する機能に影響を与えました。 （FORMS-23971）
* モバイルデバイスでは、アダプティブFormsのテーブルコンポーネントが上部に非表示のヘッダーを表示し、スクリーンリーダーがコンテンツを誤って読み上げてしまいました。 この影響を受けたユーザーは、ナビゲーションをスクリーンリーダーに依存しています。 （FORMS-23754）
* Granite:InternalAreaとしてフラグ付けされたリソースタイプを参照するアダプティブ Formsに基づくコアコンポーネントベースの問題が発生し、オンプレミス Forms アドオンのいくつかのGranite コンポーネントの機能に影響を与えました。 （FORMS-23632）
* AEM 6.5 LTS SP1へのアップグレード後にフォームの送信が失敗する。 com.adobe.cq.social.commons.CollabUtilが見つからず、JSPのコンパイル エラーとメールアクションのエラーが発生しました。 （FORMS-23457）
* 基盤コンポーネント ベースのアダプティブ Formsで、Captchaが正しく変換されない問題が発生しました。 これは、英語圏のユーザー以外のユーザーがフォームを正確に入力する能力に影響を与えました。 （FORMS-23426）
* SAXParseException: &quot;Content is not allowed in prolog&quot; （HTTP 500）。 この問題は、事前入力データ XMLのnull値が原因で発生し、サーバーサイド XMLの解析が失敗しました。 （FORMS-22633）
* ユーザーは、Web コンテンツアクセシビリティガイドライン（WCAG）監査でAdaptive Formsが失敗した。 その理由は、フォームのタブナビゲーションマークアップが無効だったからです。 つまり、リスト以外の要素は、リストの直接子としてレンダリングされ、リスト項目のみが許可されます。 この問題により、アクセシビリティバリデーターや、法的または社内のコンプライアンス要件を満たす必要がある影響を受ける組織にフォームが渡されませんでした。 （FORMS-22101）
* ユーザーは、レコードのドキュメント（DoR）/送信PDFでアクセシビリティの問題を経験しました。空のフォームフィールドがフォーム要素としてタグ付けされていませんでした。 これにより、スクリーンリーダーが困難になり、ユーザーが効果的にフォームを操作して記入する能力に影響を与えました。 （FORMS-21989）
* フォームの読み込み中に、サブパネル内のコンポーネントの脚注が表示されない問題が発生しました。 この問題は、脚注を含む項目がページの最後のコンポーネントであった場合に発生しました。 （FORMS-21925）
* AEM Forms エディターでコンポーネントを選択する際に問題が発生しました。 タブ間を移動して最初のタブに戻ると、一部のコンテナが選択不可になり、簡単に識別および操作できなくなります。 （FORMS-21814）
* ユーザーは、Adaptive Forms ダッシュボードでセキュリティ上の脆弱性を経験しました。 具体的には、startpointcontrol.js ファイルでクロスサイトスクリプティング（XSS）の問題が特定されました。これにより、悪意のあるスクリプトが実行される可能性があります。 （FORMS-20679）
* JBoss® EAP 8上のAEM Forms 6.5 LTS クラスターのデプロイメントでは、`domain/configuration/domain_oracle.xml`、`domain_mysql.xml`、`domain_mssql.xml` ファイルに、無効なXMLを引き起こし、Domain Controllerの起動を妨げる重複した`<security>` タグが含まれなくなりました。 （FORMS-24687）
* ターンキーモードでは、新しいインストールとアップグレード時にデータベースポートの更新が正しく適用されるようになりました。 新規インストールモードでは、使用可能なすべてのポートから選択でき、アップグレードモードでは、lc_turnkey.xmlで更新されたデータベースポートがアップグレードプロセス中に正しく参照されます。 （FORMS-24689）
* Linux®でJBoss® EAP 8.0を設定する際に、Windowsで変更されたシェルスクリプトが、CRLF行の終了によって`/bin/sh^M: bad interpreter or $'\r': command not found` エラーを引き起こすことがなくなりました。 （FORMS-24688）
...
* AEMは、生成された`WEB-INF/resources/provisioning/model.txt`の`contributor` グループに対する重複したACL エントリを回避するようになりました。 WAR出力に1つの一貫したACL ブロックが含まれるようになり、レビュー中に権限の相違が混乱するのを防ぐことができます。 （GRANITE-63269）
...
* `/system/console` アクセス中にFelix Web コンソールで断続的な`NullPointerException` エラーがスローされなくなりました。 更新されたServiceTracker処理は、null トラッカー状態を防ぎます。 コンソールのログインとナビゲーションは、繰り返しリクエストおよび自動検証の間も安定したままです。 （GRANITE-61042）
...
お客様は、現在のデプロイメントで機能を使用しているかどうかを確認することをお勧めします。 提供された代替案を使用するように、実装を変更する計画を立てる。
...
| Sites | [SPA Editor](/help/sites-developing/spa-overview.md) | AEMでヘッドレスコンテンツを管理するための優先エディターは、次のとおりです。<br>- [&#x200B; ビジュアル編集のためのユニバーサルエディター](/help/sites-developing/universal-editor/introduction.md)。<br>- [&#x200B; フォームベース編集のためのコンテンツフラグメントエディター](/help/assets/content-fragments/content-fragments-managing.md)。 | 6.5 LTS GA |
...
* Configuration Managerでは、AEM Forms 6.5 LTS JEE ターンキーカスタムモードのBootstrap中に、モジュールが選択されていないか、制限されたコンポーネントのみが選択されている場合、データベースの初期化が失敗します。 失敗は、依存関係が欠落しているため（xalan-2.7.2.jar）、エラーが発生します。 adobe-livecycle-jboss.ear\libにJAR ファイルを追加すると、問題が解決します。 （FORMS-24690）
* JBoss® EAP 8で実行されているForms JEE LTS デプロイメントで、Reader Extensions UIが内部サーバーエラーで失敗する場合があります。 （FORMS-24894）
* JBoss®で動作しているForms JEE LTSでは、メール関連の機能が失敗する場合があります。 電子メール機能を使用しようとすると、サーバーが`Error IMAPProvider not a subtype`と同様のエラーをログに記録する可能性があります。 （FORMS-24892）
* Linux® プラットフォームでは、Configuration Managerを実行する前に、`LFS_Foundation.properties`の`OSFileSetIntendedFor` プロパティを正しく設定する必要があります。 更新されない場合、設定がLinux®向けに適切にカスタマイズされない可能性があり、ランタイムまたはデプロイメントの問題が発生する可能性があります。 この問題を解決するには、インストーラーを実行した後、Configuration Managerを実行する前に、`configurationManager/config/solcomp/`に移動し、`LFS_Foundation.properties`を開いて`OSFileSetIntendedFor=Linux`を設定し、ファイルを保存してからConfiguration Managerを実行します。 （FORMS-24741）
...
* HTTP 400応答コードを使用したヘルスチェックの失敗。
* DispatcherとAEMのインスタンス間のトラフィックが破損している。
* Dispatcherを通じてコンテンツを適切に提供できない。
* Dispatcher設定でIP アドレスでHTTPSを使用すると、接続エラーが発生する。
* HTTPS + IP経由で接続する際にHTTP 400 「Invalid SNI」エラーが発生する。

**影響を受ける環境**

* Dispatcher設定を使用したAEMのデプロイ。
* SSLのみの機能が有効になっているシステム。
* AEM インスタンスへの`https + IP`接続メソッドを使用したDispatcher設定。
...
次のテキストドキュメントには、この[!DNL Experience Manager] 6.5 LTS、Service Pack 2 リリースに含まれているOSGi バンドルとコンテンツパッケージが一覧表示されています：<!-- UPDATE FOR EACH NEW RELEASE -->

* [Experience Manager 6.5 LTS、サービスパック 2](/help/release-notes/assets/65lts_sp2_bundles.txt) <!-- UPDATE FOR EACH NEW RELEASE -->に含まれるOSGi バンドルの一覧
* [Experience Manager 6.5 LTS、サービスパック 2](/help/release-notes/assets/65lts_sp2_packages.txt) <!-- UPDATE FOR EACH NEW RELEASE -->に含まれるコンテンツパッケージの一覧
