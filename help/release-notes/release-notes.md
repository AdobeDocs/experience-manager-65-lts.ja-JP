...
* WebSphere® Liberty Profile （WLP）のサポートが追加されました。 WLPは、Oracle DatabaseおよびIBM® Sumeru JDK 21でのみサポートされています。

* ビジュアルルールエディターでのユーザーエクスペリエンスが向上しました。 このアップデートには、次が含まれます。

  * 更新されたルールのステータスを表示するには、保存後に概要ビューが自動的に再読み込みされます
...
* ルールエディターに「レコードのドキュメントをダウンロード（DoR）/フォーム送信」を標準ボタンとして追加しました。 この機能強化により、顧客はカスタムコードを記述することなく downloadDoR 機能を使用できるようになり、ユーザビリティと効率性が向上します。 （FORMS-21263）
...
* Sites エディターで、ページオーサリング中に想定される動作が中断され、コンポーネントのインタラクションに一貫性がなくなった問題を修正しました。 作成者は、標準的な編集タスクを妨げて、ワークフローの効率が低下する、信頼性の低い UI 応答を受信しました。 このアップデートにより、エディターロジックが改善され、影響を受けるコンポーネント間で安定した予測可能なインタラクションが復元されます。 （SITES-35227）
...
* キーボードナビゲーション中に、「日付を設定」リンクと「タイムワープを終了」リンクに、フォーカスインジケーターが表示されませんでした。 UIでコントラストの高い明確なフォーカススタイルがレンダリングされるようになったため、ユーザーはアクティブリンクを簡単に識別できるようになりました。 （SITES-25232）
* ティーザーモーダルヘッダーは、キーボードユーザーによるダイアログボックスの移動をブロックしなくなりました。 キーボードコントロールで選択、移動、ドロップ操作が可能になり、スクリーンリーダーの使いやすさと全体的な操作性が向上しました。 （SITES-25226）
...
* ページ更新時の不規則な動作を防ぐために、Sites オーサリングインターフェイスでのコンポーネントの処理を調整しました。 この欠陥により、エディターの応答が予測不能になり、日常的なコンテンツの変更が妨げられ、ワークフローの効率が低下しました。 このアップデートは、エディターロジックを期待されるインタラクションパターンに合わせて調整し、オーサリングアクティビティ中に信頼できるパフォーマンスを提供します。 （SITES-35078）重要
...
* 作成者が「現在のページとサブページを昇格」を使用してサブセクションを昇格した際、ローンチプロモーション範囲がドリフトしました。 また、AEM では、無関係なページが昇格され、予期しないライブサイトの変更が発生しました。 この修正により、選択したサブツリーのみが昇格されるように、ローンチ範囲の計算が修正されます。 （SITES-38315）
...
* コンテンツフラグメントは、新しい依存バージョンと不要なGraphQL ライブラリのパッケージ化を開始します。これにより、AEM 6.5 LTSの統合が複雑になりました。 この変更により、依存関係のバージョンが AEM 6.5 LTS のベースラインと一致させられ、未使用の GraphQL の依存関係がストリップされます。 バンドル解決の一貫性が維持され、コンテンツフラグメントの起動が安定したままになりました。 （SITES-35295）
* AEM Launches は、6.5 LTS 分岐用の専用の Jenkins パイプラインを実行するようになりました。 このパイプラインは夜間にビルドを実行し、失敗アラートをメールで送信します。 これにより、テストの範囲を拡大し、リグレッションを早期に特定できます。 （SITES-35293）
...
* 通常のページオーサリングワークフローを妨げる一貫性のない動作を解決するために、Sites エディターを改良しました。 作成者は、コンテンツの更新が妨げられ、編集の信頼性が低下した、予期しないコンポーネントのインタラクションが発生しました。 この変更により、一貫性のある UI 状態の管理が復元され、影響を受けるシナリオ間でオーサリングアクションの予測可能な実行が確保されます。 （SITES-35138）
...
アセットの関連付けは、スペースを含むファイル名で機能するようになりました。 更新された関連付けクライアントロジックは、スペースを含むパスを正しく処理し、リレーション選択中の `undefined` ソースエラーを回避するようになりました。 関連付けダイアログボックスが UI のハングやスピナーなしで開き、関連を保存するようになりました。 DAM利用者は、ファイル名を変更することなく、アセットリレーションを作成、派生、削除できます。 （Assets-56418）
...
* Adaptive Forms ダッシュボードでセキュリティの脆弱性が発生しました。 具体的には、startpointcontrol.js ファイルでクロスサイトスクリプティング（XSS）の問題が特定されました。これにより、悪意のあるスクリプトを実行できます。 （FORMS-20679）
...
* Turnkey モードでは、新規インストール中およびアップグレード中に、データベースポートの更新が正しく適用されるようになりました。 新規インストールモードでは、ユーザーはすべての使用可能なポートから選択し、アップグレードモードでは、アップグレード中にlc_turnkey.xmlで更新されたデータベースポートが正しく参照されます。 （FORMS-24689）
* Linux®でJBoss® EAP 8.0を設定する際に、Windowsで変更されたシェルスクリプトが/bin/sh^M：不正なインタープリターまたは$&#39;\r&#39;:CRLF行の終了によりコマンドが見つからないというエラーが発生する問題がなくなりました。 （FORMS-24688）
* JBoss® EAP 8 上で実行されている Forms JEE LTS デプロイメントでは、Reader Extensions UI が内部サーバーエラーで失敗する場合があります。 （FORMS-24894）
* Linux®では、Forms JEE LTS Configuration Managerがアンセットまたは不正な`OSFileSetIntendedFor`で実行された際に、ランタイムまたはデプロイメントの問題が発生しました。 値を`configurationManager/config/solcomp/LFS_Foundation.properties`に設定すると、Linux®向けに設定が正しくカスタマイズされなくなります。 インストール後、Configuration Managerを実行する前に、そのファイルに`OSFileSetIntendedFor=Linux`を設定します。 （FORMS-24741）
...
* JMX コンソールとWeb コンソールは、コンソール CSS リソース用にContent-Type: text/css ヘッダーを送信するようになりました。 厳密な MIME チェックにより、スタイルシートの読み込みがブロックされなくなったので、`/system/console/jmx` UI は通常のスタイルでレンダリングされます。 （GRANITE-63677）
...
CRXDE Lite では、サービスパックのアップグレード後、JSP ファイルを開いた際に空白のタブが表示されなくなりました。 AEMには、一致するCodeMirror コアとアドオンコードが含まれるようになり、致命的なブラウザーエラーを防ぎ、エディターが機能し続けます。 （GRANITE-64333）
...
* Sling は、包括的な用語と新しい設定 PID を使用するように、管理ログインの許可リストを更新します。 この変更は、Sling JCR Base 3.2.0 と一致しています。 （GRANITE-63756）
  ...
  * 非推奨（廃止予定）の許可リスト設定と新しい許可リスト設定を同時に実行しないでください。 設定が混在すると、曖昧さが生じ、意図しない動作を引き起こす場合があります。 AEM 6.5 LTS SP2に移行する場合は、非推奨のPIDを削除します。
