...
* FORMS-24687: JBoss EAP 8上のAEM Forms 6.5 LTS クラスターのデプロイメントでは、`domain/configuration/domain_oracle.xml`に、無効なXMLを引き起こし、Domain Controllerの起動を妨げる重複した`<security>` タグが含まれなくなりました。

* FORMS-24689:In ターンキーアップグレードモードで、`lc_turnkey.xml`のデータベースポートを更新すると、アップグレード中に正しく適用されるようになり、古いポート値を参照しなくなりました。
...
* **FORMS-24690:** Configuration Managerでは、AEM Forms 6.5 LTS JEE ターンキーカスタムモードのブートストラップ中に、モジュールが選択されていないか、制限されたコンポーネントのみが選択されている場合、データベースの初期化が失敗する可能性があります。 失敗は、依存関係が欠落しているため（xalan-2.7.2.jar）、エラーが発生します。 adobe-livecycle-jboss.ear\libにJAR ファイルを追加すると、問題が解決します。
