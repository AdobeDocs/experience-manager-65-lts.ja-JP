...
* 顧客がレターをプレビューする際、「単語単位の金額」フィールドの表示や数値の更新が正しく行われず、整列が正しくなくなり、コンテンツにスペースが欠落します。 （FORMS-18437、FORMS-17330、FORMS-18209、FORMS-18557、CTG-4150848、FORMS-19614、LC-3922004）
* 顧客が RHEL で保存済みのレターをプレビューすると、コンテンツが誤って配置され、スペースが見つからず、「x」のような予期しない文字が表示されます。 （FORMS-18422）（FORMS-17641）
* ユーザーが AEM Forms のタブ間を移動すると、最初のタブでコンポーネントを選択しても応答しなくなります。（FORMS-18345）
* ユーザーが WebToPDF オプションを使用してHTML ファイルをPDFに変換すると、出力PDFにメタデータタグやタイトルタグを含むヘッダーセクションが欠落する。 （FORMS-18223、FORMS-17835、FORMS-19642、FORMS-18224）
...
* エラーメッセージが表示されずに、保存したドラフトと送信の機能が失敗する問題が発生します。 （FORMS-18069）
* XSD ベースの基盤コンポーネントからコアコンポーネントに移行すると、JSON スキーマのクロスファイル参照の実装が妨げられ、アダプティブ Formsの移行に影響を与えます。 （FORMS-18065）
...
* OData を操作すると、null ポインター例外が発生する。 これにより、データ取得プロセスが中断されます。 （FORMS-20348）
* オープンソースの Java ライブラリである Guava の削除後に、google.common.collect ライブラリを削除しました。 このアップデートにより、アダプティブ Formsを使用する大規模法人のお客様は、より高い互換性とパフォーマンスを得ることができます。 （FORMS-17031）
...
* /conf/global フォルダーに複数のエントリが存在する場合に、各フォームに正しい設定がバインドされることを確認し、AEM Formsの CAPTCHA 設定処理を改善しました。 設定コンテナが明示的に選択されていない場合に、誤った CAPTCHA 設定を意図せずに使用することを防ぎます。（FORMS-19239）
