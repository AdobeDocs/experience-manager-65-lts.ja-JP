...
* **開発環境**
新しい機能を開発したり、大きな変更を加えたりすることができます。 開発者ごとに開発環境（個人のシステム上でのローカルインストール）を使用して作業することをお勧めします。

* **オーサーテスト環境**
変更を検証します。 テスト環境の数は、プロジェクトの要件によって異なります（個別の QA 用、統合テスト用、ユーザー受け入れテスト用など）。

* **テスト環境を公開**
主に、ソーシャルコラボレーションユースケースや、オーサーインスタンスと複数のパブリッシングインスタンス間のインタラクションをテストするためのものです。

* **本番環境の作成**
コンテンツを編集できるようになります。

* **本番環境を公開**
パブリッシュされたコンテンツを提供できます。
...
16 GB 以上のヒープを割り当て、[Camera Raw パッケージ](/help/assets/camera-raw.md)を使用した Raw 画像の取り込み用に、[!UICONTROL DAM アセット更新]ワークフローを設定します。
