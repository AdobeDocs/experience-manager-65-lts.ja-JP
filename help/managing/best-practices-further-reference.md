...
オーサー環境とパブリッシュ環境では、ターゲットオーディエンスが異なるので、目標とする応答時間が異なります。
...
>[!NOTE]
>
>現実的な条件下で本番環境でのチューニング、最適化、ユーザートレーニングを可能にするため、プロジェクトの立ち上げを&#x200B;**ソフトローンチ**（利用を制限、複数回繰り返し）と&#x200B;**ハードローンチ**（完全に利用可能 - ライブ状態）とに分割。
...
* **インフラストラクチャ**
   ...
   * スプリント中にテストと最適化が行われるように計画します。
   * 安定化と最適化のフェーズを計画します。
   * できるだけ早く本番環境にデプロイします（運用チームがシステムをセットアップして経験を積めるようにします）。
