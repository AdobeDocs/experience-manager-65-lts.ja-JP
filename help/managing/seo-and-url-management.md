...
* URLは、人間が読みやすければ読みやすいほど優れています。 URLにキーワードが含まれていると、価値が高まります。
...
1. Dispatcher フラッシュ エージェントが&#x200B;`/content/my-brand/my-page`**の無効化リクエストを送信します。** Dispatcherでは、このパスにページがキャッシュされていないため、古いコンテンツはキャッシュされたままになり、古いコンテンツは古い状態になります。
...
1. **SlingResourceResolver ルール**
   ...
   Web コンソール（localhost:4502/system/console/configMgrなど）を使用して、Sling リソースリゾルバーを設定できます。
